...
const {slugify} = require('@/lib/mdxUtils');
...
exports.getRecordObject = function getRecordObject(
    ...
    //Update link forEach heading for sectionLevel
    ...
    if (splitSectionLevel) {
        ...
        headers.forEach((header, index) => {
            const fileRecord = {
                title: header,
                link: `${link}#${slugify(header)}`, //slug(header)
                platformName: platform,
                objectID: `${link}#${slugify(header)}`, //slug(header)
                headers: [header],
                keywords: [],
...
exports.createRecords = function createRecords(
    ...
    folderPaths.map((folderPath) => {
        ...
        if (subFolderPaths.length) {
            ...
            contents.forEach((content) => {
                if (content.includes('.mdx') && content.slice(-7) === 'faq.mdx') {
