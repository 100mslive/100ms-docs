...
exports.getLatestVersion = function getLatestVersion(content, folderPath, basePath) {
    ...
    const platformsWithLatestHeaders = ['JavaScript', 'React Native', 'Flutter', 'Android'];
    return {
        platform: platform === 'JavaScript' ? 'Web' : platform,
        version: platformsWithLatestHeaders.includes(platform)
            ? headers[1]
            : platform === 'Server-side'
            ? headers[0]
            : headers[0].split('-')[0].split(' ')[0],
        date: platformsWithLatestHeaders.includes(platform)
            ? headers[1]
            : platform === 'Server-side'
            ? headers[0]
            : headers[0].split(' - ')[1]
