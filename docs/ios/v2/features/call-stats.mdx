...
/// This callback provides stats for a local video track. In case of simulcast enabled room 
/// the localVideoStats param will contain an entry for each simulcast layer.
...
class HMSLocalVideoStats {
	...
	// Simulcast layer id in case of simulcast enabled room. See HMSSimulcastLayer enum for possible values
	var simulcastLayerId: NSNumber?
}
```
