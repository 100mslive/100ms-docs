...
1. Check if the screen is added as a local track for the user
2. Maintain a state variable when you successfully start and stop screen-sharing
...
<iframe allow="fullscreen;" width="800" height="460" src="https://www.youtube.com/embed/wMGfYtytAEc"></iframe>

Example project shown in the walkthrough: https://github.com/100mslive/100ms-ios-sdk/tree/main/ScreenSharingExample
...
There are 2 approaches that we suggest for checking if user is sharing screen:
1. Check if the screen is added as a local track for the user
2. Use the app group user defaults to store the state of screen sharing from broadcast extension (using broadcastStarted and broadcastFinished functions)
