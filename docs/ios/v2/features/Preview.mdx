...
Preview screen is a frequently used UX element which allows user to check if their input devices are working properly and set the initial state (mute/unmute) of their audio and video tracks before joining. 100ms SDK provides an easy to use API to back this feature. Additionally SDK will try to establish a connection to 100ms server to verify there are no network issues and that the auth credentials are valid so that if everything is in order the subsequent room join is pretty much instant.
...
You would need same config object that you would pass to [join API](Join). The `previewDelegate` is an object conforming to `HMSPreviewListener` protocol that has two callbacks:
