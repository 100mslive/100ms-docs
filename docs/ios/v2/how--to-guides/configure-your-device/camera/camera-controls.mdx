...
If you want to modify camera settings like zoom level, flashlight, night-mode, HDR etc. you can use camera control API to do so.
...
100ms SDK gives you safe access to current AVCaptureDevice object that represents camera being used on the iOS device. You change settings on this AVCaptureDevice object to your liking.
...
For example a zoom level toggle can be implemented like below:
...
```
Flash light toggle can be implemented like below:
...
100ms iOS SDK gives you access to underlying camera device in the form of AVCaptureDevice. This means you have access to all the settings available on AVCaptureDevice object. To reference what all is available go to Apple's documentation [here](https://developer.apple.com/documentation/avfoundation/avcapturedevice).
