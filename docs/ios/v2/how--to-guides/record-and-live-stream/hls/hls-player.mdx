...
func on(join room: HMSRoom) {
  let hlsUrl = room.hlsStreamingState.variants.first?.url
  hmsPlayer.play(hlsUrl)
}

// Case 2 - listen to hlsStreamingStateUpdated update to handle starting and stopping of hls playback
func on(room: HMSRoom, update: HMSRoomUpdate) {
  ...
  case .hlsStreamingStateUpdated:
    if let hlsUrl = room.hlsStreamingState.variants.first?.url {
      ...
      // Stop the playback if url is empty - that means streaming is stopped
      hmsPlayer.stop()
    }
  }
}
