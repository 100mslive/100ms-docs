...
## 0.6.3 - 2023-01-30
### Added
Added API to get AVCaptureDevice from localvideotrack
Added a way to take a photo while video capture for a high res stabil…
Added decodedString on AVMetadataItem for getting the payload string
Added setting to disable PiP
### Fixed
Bringing back reverted change: "IOS-498 Implement disconnected state timeout for publish connection""
Fix IOS-513 No camera/mic if joined without preview
IOS-512 Add websocket endpoint to offline event payload## 0.6.1 - 2023-01-19
### Added
