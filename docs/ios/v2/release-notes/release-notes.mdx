...
## 1.17.3 - 2026-09-07
### Fixed
- Crash when a device has no available cameras
...
- Moved to new HMSWebRTC version to resolve naming conflicts with other WebRTC dependant libraries



## 1.16.8 - 2024-12-26
### Added
- Time limit param in startConnectivityCheck

### Fixed
- Completion callback not fired when stopConnectivityCheck is called
- Echo during connectivity check
...
## 1.16.3 - 2024-09-26
### Fixed
- Fixed audio bitrate units in analytics


## 1.16.2 - 2024-09-20
### Fixed
- Analytics not getting sent for short sessions


## 1.16.1 - 2024-09-02
### Fixed
- Screen share not recovering after long session migration
- Memory leaks
...
- Jitter and packets lost stats for local tracks
- Analytics improvements


### Fixed
- Calling leave after preview without joining results in SDK not releasing preview tracks


## 1.15.0 - 2024-07-22
### Added
- New operating mode property based interface for HMSVirtualBackgroundPlugin
- Virtual Background support in Room Layout
...
## 1.14.0 - 2024-07-04
### Changed
- Join time optimisations


## 1.13.0 - 2024-06-28
### Added
- Pre call diagnostics API
- Noise cancellation permission in HMSRole and NC default state in room layout
- Analytics improvements

### Fixed
- Whiteboard permission parsing
- No audio sent when changing role from non-publishing to publishing 
- Poll started callback does not come for locally created polls


## 1.12.0 - 2024-06-07
### Added
- imageDataSource API for virtual background plugin.
- start/stop live transcription APIs 
- search peers by name API for large rooms


## 1.11.0 - 2024-05-17
### Added
- proxy/turn configuration API in HMSConfig


## 1.10.0 - 2024-05-08
### Added
- Transcription state in HMSRoom

### Changed
- Blur plugin performance improvements

### Fixed
- Freeze on plugin state toggle on some devices


## 1.9.0 - 2024-04-19
### Added
- Transcription feature in conferencing calls
- VP9 codec support
- Whiteboard feature

### Fixed
- Peer count update does not come for empty room


## 1.8.0 - 2024-03-27
### Added
- type property in HMSPeer for SIP peers support.
- Hand raise configuration in room layout API
### Fixed
- Participant video appears stretched in some scenarios.
...
### Fixed
- Fix poll vote count reset after disconnection


## 1.6.0 - 2024-02-27
### Added
- playlist type property in HMSHLSVariant
- Support for header and description in room layout API


## 1.5.1 - 2024-02-02
### Added
- Skip preview flag in room layout API


## 1.5.0 - 2024-01-19
### Added
- Color controls video plugin


## 1.4.2 - 2023-12-28
### Added
- Quiz leaderboard summary API


## 1.4.1 - 2023-12-14
### Added
- Room layout support for skipping preview in bring on stage feature
...
### Fixed
- Occasional crash when subscriber analytics are enabled


## 1.3.0 - 2023-11-16
### Added
- State property for streaming and recording room states.

### Fixed
- createdBy property not populated for HMSPoll


## 1.2.1 - 2023-11-10
### Fixed:
- Auto simulcast layer selection not picking highest layer for large tiles.
...
#### Breaking change
- Removed tracksErrorAudioShouldBeSilenced error that was used to signal that other app is playing audio that cannot be mixed, it is now up to the app to detect for example using CallKit
