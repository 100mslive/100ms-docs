...
[HLS Streaming](/ios/v2/foundation/live-streaming) allows for scaling to millions of viewers in near real time. You can give a link of your
...
> on YouTube, Twitch etc., please have a look at our RTMP streaming docs [here](/ios/v2/how--to-guides/record-and-live-stream/rtmp-recording).
...
```swift
hmsSDK.startHLSStreaming() { didStart, error in
...
To use your own web-app UI for HLS, you'll need to pass in a meeting URL. The 100ms bot will open this URL to join your room, so
...
```swift
let config = HMSHLSConfig(variants: [HMSHLSMeetingURLVariant(meetingURL: meetingURL, metadata: "tag for reference")])
hmsSDK.startHLSStreaming(config: config) { didStart, error in
    print(didStart, error)
}
```
...
let recordConfig = HMSHLSRecordingConfig(singleFilePerLayer: true, enableVOD: false)
...
```swift
@objcMembers public class HMSHLSStreamingState : NSObject {
    public let running: Bool
    public let variants: [HMSHLSVariant]
}
```
...
2. In the `on(room: HMSRoom, update: HMSRoomUpdate)` callback of `HMSUpdateListener`.
   The `HMSRoomUpdate` type will be `hlsStreamingStateUpdated`.
</Tab>

<Tab id='sample-code-1'>
You can call `startStreaming` method on HMSRoomModel instance to start streaming the room.
```swift
try await roomModel.startStreaming()
```

## How to stop streaming
To stop streaming the room, call `stopStreaming` method.
```swift
try await roomModel.stopStreaming()
```

## How to know when room is getting streamed
Check the `isBeingStreamed` on HMSRoomModel instance to know if room is getting streamed.
```swift
roomModel.isBeingStreamed
```

## How to play the hls stream from the room on viewer side
You instantiate `HMSHLSPlayerView()` instance to render the HLS stream from the room you are in.
```swift
HMSHLSPlayerView()
```
 <details>
...
struct MeetingView: View {
    ...
    @ObservedObject var roomModel = HMSRoomModel(roomCode: "qde-mik-seb")
