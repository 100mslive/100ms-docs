...
## How to show video player UI that plays HLS stream of the Room
...
## How to play HLS stream when you have a URL of hls stream
...
You can use a view builder as overlay parameter to `HMSHLSPlayerView`.
```swift
HMSHLSPlayerView() { player in
    // player here is an instance of HMSHLSPlayer
    // custom overlay controls for HLS playback
    HStack {
        // pause hls stream
        Button(action: {
            player.pause()
        ...
        case .inMeeting:
            VStack {
                if roomModel.hlsVariants.first?.url != nil {
                    ...
                    .onPlaybackStateChanged { playbackState in
                        // handle HMSHLSPlaybackState enum here
                        // where HMSHLSPlaybackState is
                        ///  @objc public enum HMSHLSPlaybackState: Int {
