...
You call **switchAudioOutput(to:)** method on HMSRoomModel instance passing in the HMSAudioOutputDevice that you would like to switch audio to. Like below:

```swift
// Switch audio to earpiece
try roomModel.switchAudioOutput(to: .earpiece)
...
No. Currently _in-built speaker_ and _in-built earpiece_ are the only two supported devices.
