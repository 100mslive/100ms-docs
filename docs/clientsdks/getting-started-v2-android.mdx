...
100ms' Android SDK supports Android API level 21 and higher. It is built for armeabi-v7a, arm64-v8a, x86, and x86\_64 architectures
...
Please use the [100ms quickstart app server](../server-side/100ms-quickstart-app-server.md) to get `TOKEN_ENDPOINT`
...
To join a room created by following the steps described [in this section](../server-side/create-room.md), clients need to create a `HMSConfig` instance and use that instance to call `join` method of `HMSSDK` . 
