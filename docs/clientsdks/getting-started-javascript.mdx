...
4. **Create a room and setup a token generation service**: To create a room, follow the steps described [in this section](../server-side/create-room.md). To setup a token generation service, follow the steps in [this section](../server-side/generate-client-side-token.md). Alternatively, you can use our [quickstart server](../server-side/100ms-quickstart-app-server.md) for this.
...
{% tab title="Room,  Peer,  Track Interfaces" %}
```jsx
interface HMSPeer {
    peerId: string;
    name: string;
