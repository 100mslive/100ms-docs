...
Autoplay error will be thrown by browser if something is played without any user interaction. The sdk throws an error with code `3008`. This can be handled in [Notifications](../web-frameworks/Getting-started-react#notifications)

Unblock browser autoplay by calling `await hmsActions.unblockAudio()` on some UI interaction (eg: button click)
