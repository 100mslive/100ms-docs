...
hmsManager.triggerOnSubscribe(); // optional
...
// The volume is a value between 0 and 100
hmsActions.setVolume(peer.audioTrack, 10);

// Get volume for audio Track
const volumeByPeerID = hmsStore.getState(selectAudioVolumeByPeerID(peer.id));
// Get volume for screenshare audio Track
const screenshareVolumeByPeerID = hmsStore.getState(selectScreenshareAudioVolumeByPeerID(peer.id));
```

## Show Current Speaker and Audio levels

We also expose audio level for all peers which is a number between 0-100. This can be used to display the current speaker as well as individual
audio levels for each peer on the UI.

```js
const onSpeakerUpdate = (speaker, prevSpeaker) => {
    console.log('speaker changed from - ', prevSpeaker.name, ', to - ', speaker.name);
};

hmsStore.subscribe(onSpeakerUpdate, selectDominantSpeaker);

// current speaker - peer with highest audio level
console.log(hmsStore.getState(selectDominantSpeaker));

// getting audio level for a particular peer
console.log(hmsStore.getState(selectPeerAudioByID(peer.id)));

// getting audio level for a particular track
console.log(hmsStore.getState(selectTrackAudioByID(track.id)));
```

## Notifications

Notifications can be used to display toast to user about activities happening in the room, such as peer join/leave or
new messages. It can also be used to know about any mid call sdk level error.
...
### Fetching role related information

<Tabs id="role-selectors" items={['Usage', 'Interfaces']} />

<Tab id="role-selectors-0">
...
const localPeerRole: HMSRole = hmsStore.getState(selectLocalPeerRole); // The HMSRole object of the local peer
const isAllowedToSubscribe: boolean = hmsStore.getState(selectIsAllowedToSubscribe); // true if the local peer is allowed to subscribe to any other role
const isAllowedToPublish: HMSPublishAllowed = hmsStore.getState(selectIsAllowedToPublish); // the user's publish permissions

const rolesMap: Record<string, HMSRole> = hmsStore.getState(selectRolesMap); // A map of all roles in the room
const roleNames: string[] = hmsStore.getState(selectAvailableRoleNames); // A list of all role names in the room
```

</Tab>

<Tab id="role-selectors-1">

```jsx
interface HMSPublishAllowed {
    video: boolean;
    audio: boolean;
    screen: boolean;
}
...
```jsx
import { selectAvailableRoleNames, selectRemotePeers } from '@100mslive/hms-video-store';

const roleNames = hmsStore.getState(selectAvailableRoleNames);
const remotePeers = hmsStore.getState(selectRemotePeers);

hmsActions.changeRole(remotePeer.id, newRoleName, force);
```

The receiver of the request can listen to incoming requests and take appropriate action.

<Tabs id="role-change" items={['Usage', 'Interfaces']} />
...
function onRoleChangeRequestReceived(request: HMSRoleChangeRequest) {
    hmsActions.acceptRoleChangeRequest(request);
    // or
    hmsActions.rejectRoleChangeRequest(request);
}

hmsStore.subscribe(onRoleChangeRequestReceived, selectRoleChangeRequest);
```
...
hmsActions.preview(previewConfig);
```

Calling `preview` will set the `localPeer`. You can subscribe to the `localPeer` update as mentioned [here](#listen-to-changes-in-peers-tracks-data)
and [render](#render-video) the video track.
...
Autoplay error will be thrown by browser if something is played without any user interaction. The sdk throws an error with code `3008`. This can be handled in [Notifications](../client-sdks/Reactive-Store-Extension-to-the-JS-SDK#notifications)

Unblock browser autoplay by calling `await hmsActions.unblockAudio()`
