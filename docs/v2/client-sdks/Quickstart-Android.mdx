...
```kotlin
    ...
    val roomState: LiveData<List<TrackPeerMap>> = _roomState
```
...
```kotlin
    ...
    override fun onError(error: HMSException) { Log.d(TAG, "Error $error") }

    override fun onJoin(room: HMSRoom) { _roomState.postValue(getCurrentRoomState()) }
