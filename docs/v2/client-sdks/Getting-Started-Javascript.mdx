...
Most modern browsers don't allow audio to be played without any interaction. When the browser throws an autoplay error, we throw an error in `onError` handler with an error code `3008`.
