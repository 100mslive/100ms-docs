...
```bash
npm install --save @100mslive/hms-virtual-background
```
import HMSBackgroundProcessor from "@100mslive/hms-virtual-background"

//Initialize a new Processor for Virtual Background, it accepts parameters as
// 1) background : 'blur' -> to blur the background
//                 'imgSource' -> image URL to replace the background
//                  'default' -> to set back default background
// 2) framerate : fps to run the process
backgroundProcessor = new HMSBackgroundProcessor({background: 'blur'|'default'|imgSource, framerate: 30})
hms.getLocalPeer().videoTrack.addProcessor(backgroundProcessor);
hms.getLocalPeer().videoTrack.removeProcessor(backgroundProcessor);
```
