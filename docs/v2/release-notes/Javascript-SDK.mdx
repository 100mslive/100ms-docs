...
-   HMSPeer object will now have `customerUserId` property that stores the `userId` from [Client Token](../server-side/generate-client-side-token.md#sample-client-side-token-generation-code).
