...
-   Changing roles mid-call [Refer Usage](../client-sdks/Getting-Started-JavaScript#changing-roles-mid-call)
-   BREAKING CHANGE: The interface of `HMSPeer` has been modified for users using the JavaScript SDK without the [Reactive Store](../client-sdks/Reactive-Store-JavaScriptx).
-   BREAKING CHANGE: For users of the [Reactive Store](../client-sdks/Reactive-Store-JavaScriptx), the `HMSPeer` interface has been modified. Use `peer.roleName` instead of `peer.role` to obtain the name of a peer's role.
