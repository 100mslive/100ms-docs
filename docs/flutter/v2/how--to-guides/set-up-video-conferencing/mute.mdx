...
## When to show mic/camera mute/unmute button on UI 

On UI we only want to show mic or camera buttons only when the peer has permissions to publish audio or video respectively.
...
///Here we will add the check to render the widget to screen or not based on [displayMicrophoneButton] variable
...
///Here we will add the check to add the widget to screen or not based on [displayCameraButton] variable
