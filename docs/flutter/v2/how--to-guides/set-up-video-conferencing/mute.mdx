...
/// [isOn] : Pass the current audio status, that is, true if unmuted and false if muted currently
...
/// [isOn] : Pass the current video status, that is, true if unmuted and false if muted currently
...
If you are looking to Mute other (remote) Peers in Room, checkout the [Remote Mute here](./remote-mute-unmute).
