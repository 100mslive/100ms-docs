...
class Meeting implements HMSUpdateListener, HMSActionResultListener{
  ...
  void onRoomUpdate({required HMSRoom room, required HMSRoomUpdate update}) {
    ...
    ///  bool isHLSRunning = room.hmshlsStreamingState?.state == HMSStreamingState.started;
...
```dart
///[running]: bool value `true` indicates that HLS streaming is running
...
///[playlistType]: Enum to differentiate whether a stream is seekable or non-seekable. For seekabable it's set to `HMSHLSPlaylistType.dvr`
///for non-seekable it's set to `HMSHLSPlaylistType.noDvr`
class HMSHLSVariant(
  String? hlsStreamUrl;
  String? meetingUrl;
  String? metadata;
  DateTime? startedAt;
  HMSHLSPlaylistType playlistType;
)
```
