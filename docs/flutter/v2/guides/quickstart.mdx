...
Once you have the above prerequisites, you can follow the below steps to create a Flutter app. In this guide, we will be using VS code but you can use any IDE that supports flutter.

-   Create a Flutter app using terminal, you can get the [Flutter SDK](https://docs.flutter.dev/get-started/install/macos#get-sdk) and use the below command:
...
4. You will see two 100ms demo URLs for two roles created when you deployed the starter kit; you can click on the 'key' icon to copy the token and update the `authToken` in "lib/main.dart" file.

<video loop="true" autoplay="autoplay" controls="controls" id="vid" muted>
    <source src="/docs/guides/update-token-flutter-android.mp4" type="video/mp4" />
</video>
<br />
...
Please follow the below instructions to test the app on android target platform:

1. Allow camera, recording audio and internet permissions by adding them to the `AndroidManifest.xml` file.
2. Add minimun SDK version (`minSdkVersion 21`) inside "defaultConfig".
...
-   You can build the app and run it in a emulator or an actual android device.

Now, after you click `join`, you should be able to see yourself. You can join the room using a browser to join as the second peer to check audio/video transactions between two or more peers.
