...
You can also check our [sample app](https://github.com/100mslive/100ms-flutter/tree/main/example).
...
Once you have created a Flutter app, you must add the 100ms Flutter SDK and permissions_handler (to handle audio/video permissions from microphone and camera) to your app.
...
-   Go to _Run > Start debugging > select a device to use_ (android emulator or android phone connected via USB).
...
-   Go to _Run > Start debugging > select a device to use_ (iOS simulator or iPhone connected via USB).

Now, after you click `join`, you should be able to see yourself (the android emulator doesn't support actual video, you can connect an actual device to see your video in real-time). You can join the room using a browser as the second peer to check audio/video transactions between two or more peers.

<video loop="true" autoplay="autoplay" controls="controls" id="vid" muted>
    <source src="/docs/guides/join-room-flutter-ios.mp4" type="video/mp4" />
</video>
<br />
...
To display a video track, first get the `HMSVideoTrack` & pass it on to `HMSVideoView` using the `setVideoTrack` function. Ensure to attach the `HMSVideoView` to your UI hierarchy. Check the [render video guide](./../features/render-video) for more information. 
...
For a complete example, see the [sample app](https://github.com/100mslive/100ms-flutter/tree/main/example) on GitHub — This uses the [provider package](https://pub.dev/packages/provider) as the state management library.

### Full-fledged example

You can also check out the full-fledged example app implementation in the 100ms Flutter SDK [GitHub repository](https://github.com/100mslive/100ms-flutter/).
