...
Metadata can be set initially in the `HMSConfig` object that's passed into the join method. This can be used to display profile pictures as the user's avatar in meetings.
...
The `HMSPeer` object contains the parameter `metadata`.

Any peer can change the metadata for themselves. Currently, one peer cannot change another peer's metadata.

To change their metadata value the peer should call `changeMetadata` on their `HMSSDK` instance.
...
![hand-raise](/docs/v2/flutter-accept-hand-raise.png)
