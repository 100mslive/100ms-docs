...
In Android, if you want to speak in sleep mode then Android OS allow it for 60 seconds by default but if you want to continue it for more than 60 seconds then we have to use 3 party plugin [flutter_foreground_task](https://pub.dev/packages/flutter_foreground_task) which allows the app to run in the foreground with a notification.
