...
When your app goes into the background, by default it can no longer access camera and publish it to other peers in the room. Also, you cannot hear audio of other participants in the call if your app is in the background without enabling Background Modes.
...
As per your use-case, apps can choose to implement an [Android Foreground Service](https://developer.android.com/guide/components/foreground-services) to ensure consistent behaviour. With a Foreground Service, you can continuously access Microphone, Camera & play Incoming Audio of other peers in the Room. 
...
### Without Android Foreground Service
...
When your app implements a Foreground Service it has steady access to capturing & playing audio & video.
