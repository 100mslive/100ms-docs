...
While a user has joined a Room they can put the in Background Mode & usually expect a certain subset of app functionality continues to work. 
...
There are 2 prominent behaviours that are mainly affected when apps transition to Background State - Publishing Audio/Video and Subscribing or Playing incoming Audio/Video.
...
When your app goes into the background, it can no longer access camera and publish it to other peers in the room. Also, you can't see video of other particiapants in the call if your app is in the background.
