...
Screen share can be rendered on the screen similar to normal videos.

Pass the screen share track in `HMSVideoView` with `matchParent` as `false` and `ScaleType` as `ScaleType.SCALE_ASPECT_FIT`.
