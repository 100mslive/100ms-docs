...
Flutter iOS provides support for sharing the entire screen of the device in the room or you can share just your App's Screen.
...
```dart
  HMSException exception = await hmsSDK.startAppScreenShare();
  if(exception == null){
    // App screen share start successfully.
  }
```
...
```dart
  HMSException exception = await hmsSDK.stopAppScreenShare();
  if(exception == null){
    // App screen share stop successfully.
  }
```

### Share The Entire Device Screen

To share entire device screen you need to create an iOS broadcast upload extension. It uses Apple's ReplayKit framework to record the device screen and delivers frame samples to your broadcast extension. You can share not only your own app but also the entire device sceeen including other apps on the device.
...
To start Screenshare from iOS devices you need to pass `iOSScreenshareConfig` to `HMSSDK` constructor as shown below.
...
```dart
// Pass the correct App Group & Preferred Extension parameters while contructing the HMSIOSScreenshareConfig object.
HMSIOSScreenshareConfig screenshareConfig = HMSIOSScreenshareConfig(
...
HMSSDK hmsSDK = HMSSDK(iOSScreenshareConfig: screenshareConfig); // Pass screenshareConfig to iOSScreenshareConfig parameter.
