...
Flutter SDK provides support for sharing the entire screen of the device with the room.
...
You also need to pass the intent from the android native side to HMS SDK in the following way :
...
A new section should be added under `Signing & Capabilities` named App Groups. Click + icon under that.
...
### Step 4 - Edit Podfile
In the ios folder of your flutter project and open `Podfile`.
...
In the terminal change directory to ios and run the `pod install` command.
...
Replace the code with the code below and pass the app group name to the respected field:
...
To start screen share, the app needs to call the `startScreenshare` method of `HMSSDK`.
...
The application needs to call the `isScreenShareActive` method of `HMSSDK`.

This method returns a `Boolean` which will be true in case ScreenShare is currently active and being used, and False for an inactive state.
...
To stop the screen share, you need to invoke the `stopscreenShare` method of `HMSSDK`
...
The peer's screen share can also be found in the `auxiliaryTracks` property of peer. `auxiliaryTracks` is of type `List<HMSTrack>?`.

To get the screen share track from `auxiliaryTracks` the implementation is as follows:

```dart
HMSTrack? screenShareTrack =
        peer.auxiliaryTracks?.firstWhere((track) => track.source == "SCREEN");
if(screenShareTrack != null){
  //The user is sharing the screen and has a screen share track.
