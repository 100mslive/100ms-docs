...
To start screen share in iOS you need to pass `App Group` and `Extension` name to `HMSSDK` before build while initialization as follows:

```dart
HMSSDK(appGroup: "group.name", preferredExtension: "your.extension.name")
```

Checkout the detailed docs [here](../features/hmssdk#initializing-hmssdk-for-advanced-use-cases)
...
After completing the setup let's see how we can start the screenshare.
