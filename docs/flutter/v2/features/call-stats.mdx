...
100ms SDK provides this data via dedicated delegate callbacks found in `HMSUpdateListener`. These will be called with a fixed interval of one second after a room has been joined. You can get stats on a per-track basis ( `onRemoteAudioStats`) or as an overall summary (`onRTCStats`) 
...
1. Implement `HMSStatsListener` wherever we want their callbacks

```dart 
class StatsReport implements HMSStatsListener{}
```

2. Call `addStatsListener` on the hmsSDK instance because we will only get updates after calling `addStatsListener` on the hmsSDK instance
...
3. Override the above methods as:
...
4. To stop listening to updates or while leaving the room we will need to remove this listener as:
