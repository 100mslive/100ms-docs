...
//OnReconnecting callback is triggered when the SDK detects a network issue or is trying to reconnect automatically on a network drop, switch etc. 
@override
void onReconnecting(){
    // Reconnecting...
...
//OnReconnected callback is triggered when the SDK has successfully recovered from a network drop, switch or a network issue etc.
@override
void onReconnected(){
    // Reconnected...
...
![Error Code:1003](/docs/v2/flutter-errorCode-1003)

* `Error Code: 4005, Cause: ICE Connection Failed due to network issue[PUBLISH]`

![Error Code:1003](/docs/v2/flutter-errorCode-4005-PUBLISH)

* `Error Code: 4005, Cause: ICE Connection Failed due to network issue[SUBSCRIBE]`

![Error Code:1003](/docs/v2/flutter-errorCode-4005-SUBSCRIBE)

## Best Practices to handle Reconnection

The SDK will send `onReconnecting` callback first if user disconnects, If user reconnects again the SDK sends `onReconnected` callback whereas if
the user fails to connect again then it sends `onHMSError`.These callbacks can be handled as follows: 

* Once the SDK sends `onReconnecting` callback,Do not allow user to interact with UI and show some loader or reconnecting indicator
* If `onReconnected` is received then switch back to room UI allowing interaction as usual.
* If the user fails to reconnect then `onHMSError` callback look for `isTerminal` parameter if it 
is set to `true` then there is no chance of recovering hence navigate to the `HomeScreen`,showing as toast or errorDialog to the user.
...
> 🗝️ Note: You can find the `errorCode` and the suggested action to be taken [here](./error-handling)