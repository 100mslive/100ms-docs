...
### How to stream device audio from the application
To start streaming device audio, the application needs to call the `startAudioShare` method of `HMSSDK`, which takes in three parameters -
...
2. The second one is one of the modes of type `HMSAudioMixingMode` in which the user wants to stream.
    ...
    - MUSIC_ONLY:  The playback audio being captured from the device will be streamed in the room
...
To stop capturing device audio and streaming into the room, call the `stopAudioShare` API and provide an `HMSActionResultListener`
