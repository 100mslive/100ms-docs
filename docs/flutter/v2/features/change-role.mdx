...
The `force` parameter in `changeRoleOfPeer`, when false, is a polite request: "Would you like to change your role from listener to speaker?" Which can be ignored by the peer. The way it works is the other peer will first receive an `onRoleChangeRequest` callback in `HMSUpdateListener`:
