...
Once you have both you can invoke `changeRoleOfPeer` for changing role of single peer in the room.
...
If you want to change role of all the peers in the room then use `changeRolesOfAllPeers` as follows:

```dart
 hmsSDK.changeRolesOfAllPeers(
 toRole: toRole, 
 limitToRoles: limitToRoles);
```

`toRole`: the HMSRole you want to switch to.
`limitToRoles`: the list of HMSRole in which you want to apply role change.

> 💡 Note: Force change is true in `changeRolesOfAllPeers` API.
