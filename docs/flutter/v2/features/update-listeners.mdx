...
abstract class HMSUpdateListener {
  ...
  ///when someone requests for a track change of yours be it video or audio this will be triggered
  ...
  ///whenever a new audio device gets connected to the device or the audio device is switched this is triggered
  ///- Parameters
...
❌ If failure, the `onHMSError({required HMSException error})` method will be invoked with the exact failure reason.
