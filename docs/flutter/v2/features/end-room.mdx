...
*reason*: reason String is the text that is passed describing why the room is being ended.
*hmsActionResultListener*:  It's the callback that would be called by SDK in case of a success or failure of `endRoom` operation
