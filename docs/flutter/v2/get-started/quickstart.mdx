...
#### Fetch token using room-code methods(Recommended) 

We can get the authentication token using room-code from meeting URL as well.
...
Now to get the room-code from meeting URL you can write your own logic or you can use the `getCode` method from [here](https://github.com/100mslive/100ms-flutter/blob/main/example/lib/service/room_service.dart)
...
> This method should be called after calling the `build` method. Also, passing user name as userId is not compulsory. You can directly pass the user name in HMSConfig while joining the room.
...
> 🔑 Note: If the user can join with an emoji in it's user name then it is recommended to not pass `userId` in `getAuthTokenByRoomCode` this may lead to crashes sometime. Although you can
>          pass it in the HMSConfig.
