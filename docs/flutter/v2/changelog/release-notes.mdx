...
- Added phoneCallState property in `HMSAudioTrackSetting`
  ...
  The `phoneCallState` property can be used to set the microphone state when you receive a phone call, i.e. to mute the microphone when a phone call is received.

  Read more about the `phoneCallState` property [here](https://www.100ms.live/docs/flutter/v2/how-to-guides/configure-your-device/microphone#phone-call-state).
