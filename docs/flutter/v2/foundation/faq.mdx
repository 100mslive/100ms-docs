...
We have `HMSStore` as our data store class then if we create the instance while navigating this will cause error while 
hot reloading.
...
To support hot reload the object should be passed in while navigating:
