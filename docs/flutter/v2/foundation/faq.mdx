...
#### My device heats up while using 100ms SDK?
...
- Videos which are not visible on screen should not be downloaded.HMSVideoView internally uses `SurfaceView` in android and `UiKitView` in iOS. It is recommended to render at most 3 to 4 videos on a Single page/screen of the app and rest should be paginated for optimum performance.
...
To mirror your device you can use tools like [SCRCPY](https://github.com/Genymobile/scrcpy#get-the-app),[VYSOR](https://www.vysor.io/download/), quick time player etc.
