...
100ms _client-side SDKs_ use App Tokens to authenticate a peer (participant) while [joining a room](/flutter/v2/features/join). Generate this token on the server-side and make it available for your client-side apps that use the 100ms SDKs.
...
private void generateHmsClientToken() {
    Map<String, Object> payload = HashMap<>();
...
For setting up management tokens for server side API requests, please refer to the [authentication guide](/server-side/v2/introduction/authentication-and-tokens) in server-side API section.
