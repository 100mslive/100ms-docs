...
### Preview Screen
The Prebuilt preview screen offers a set of components designed to enhance the initail set-up before joining a room. Choose a role to kickstart your customization journey.
...
##### Noise Cancellation State
	- Turn the toggle to define the initial state for Noise Cancellation.
	  ...
	  <Note> 
	  Noise Cancellation should be enabled from 'Advanced Settings' under the Template to enable this setting. Learn more on enabling this feature from [here](/get-started/v2/get-started/features/noise-cancellation#enabling-the-noise-cancellation)
	  </Note>
...
-	###### Allow pausing chat
	Enabling this feature allows the selected role to pause the chat messages during the session, once paused no peer can send any messages in the chat until it is unpaused.
