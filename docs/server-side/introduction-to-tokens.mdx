...
excerpt: Introduction to tokens
date: 31-12-2020
---

# Introduction to tokens

100ms Access Tokens are based on the [JSON Web Token](http://jwt.io/) standard. You can read about the details of the JWT format for Access Tokens in this section, but if you're using \*\*\*\*our [quickstart app server guide](100ms-quickstart-app-server.md) you can skip this section

## JWT Format

Each Access Token is a [JWT](http://www.jwt.io/), which is an encoded JSON object with three parts: the header, the payload, and the signature. The following is a sample 100ms server-side JWT token
...
If we inspect it [with the debugger at jwt.io](http://www.jwt.io/), we can further explore its content.
...
-   `iss` is the issuer - for all 100ms tokens, your `customer_id` is used as issuer \(This is optional for 100ms tokens\)
...
-   `access_key` is the 100ms key .used to generate the token. This can be your `app_access_key`
