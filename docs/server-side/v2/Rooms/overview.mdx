...
A room is a virtual space where one or more participants communicate with each other. It is a central entity that provides real-time communication capabilities for any video conferencing or live streaming session.
...
> **Note**: Enabled/Disabled room -- A room will be enabled/disabled if the room is enabled/disabled via API or dashboard.
