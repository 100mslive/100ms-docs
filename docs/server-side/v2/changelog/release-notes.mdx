...
-   Added the [API reference documentation](/server-side/v2/deprecated/templates-and-roles#api-reference) for legacy APIs(V1).
...
-   In the template object, the fields \_id, and customer, supported with the [Legacy API (V1)](/server-side/v2/deprecated/templates-and-roles#api-reference), is deprecated. You can continue to use them as id and customer_id with the new API. Refer to the [template object section](/server-side/v2/policy/template-object#template-object) for more information.
-   In the room object, the field template supported with the template name in the [Legacy API (V1)](/server-side/v2/deprecated/room#body-paramters) will not be supported in the new API. Instead, you can use the [template_id](/server-side/v2/Rooms/create-via-api#main-arguments) field.

> **Note**: We recommend you use our new API to leverage the new functionalities and features and stay up to date with future releases. <br/> <br/>The Legacy API (V1) will remain fully functional until the subsequent notice. Currently, there's no end of life for them; you can continue to check the docs for the Legacy API (V1)s [here](/server-side/v2/deprecated/templates-and-roles).
