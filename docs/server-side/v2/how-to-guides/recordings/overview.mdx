...
If you wish to automatically start 'Room composite' recordings for all your rooms and sessions when the first peer joins, you can enable the **'Enable auto-start on room join'** toggle in the **Recording** settings on the template dashboard. Note that enabling this will disable HLS and RTMP streaming for that template if you have them enabled.
