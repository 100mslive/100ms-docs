---
title: Live Streaming with RTMP Ingestion
nav: 8
---
...
<StepsToc
    ...
    descriptions={[
        ...
        "Starting, previewing, interacting, and ending your live streams."
    ]}
/>
...
Use the [Create Stream Key API](/server-side/v2/api-reference/stream-key/create-rtmp-stream-key) to create the stream key for a specific room. 

Existing Stream key for a room can be fetched using the [Get Stream Key API](/server-side/v2/api-reference/stream-key/get-rtmp-stream-key).  

To instantly stop ingestion on any room, the stream keys can be disabled using the [Disable Stream Key API](/server-side/v2/api-reference/stream-key/disable-rtmp-stream-key). Alternatively, the room can be marked as disabled. For rooms that are set to disabled state, the stream key linked to the room also becomes inactive. Once the room is enabled, the linked stream key becomes active again. 
...
|[`ingest.start.failure`](ab) | Connection with 100ms Ingest server has failed. The exact reason can be inferred from the error_code and error_message |
|[`hls.failure`](ab) | Any internal failure in the live stream start up process.  |
| [`ingest.end.success`](ab) | Connection with 100ms Ingest has successfully terminated. This is sent on stopping the stream from OBS / external application. Our server will internally signal to stop the live stream on the room |
| [`hls.stopped.success`](ab) | Live Stream has stopped. Playback is not active from this point onwards|
|[`hls.recording.success`](ab) | Recording for the live stream is successfully generated |
...
1. Since the media is ingested from an external source and directly streamed through HLS, the viewers of the HLS stream will not be able to see anything going inside the conferencing (WebRTC) room.
2. The peers in the conferencing (WebRTC) room (if any) will not be able to watch the incoming RTMP-Ingested media stream.
3. The broadcaster role created for the given template [above](jnfdv) can chat with the viewers, but the experience will be sub-optimal as the stream is running couple of seconds delayed.
...
1. Continue using the same room for your live streams. The older stream key and RTMP URL will be valid to start future live streams as well.
2. Create a new stream key but continue using the same room. Effectively, there are two methods to reset or regenerate a new stream key.
3. Disable the stream key or lock / disable the room. This can be accomplished using following two methods.
