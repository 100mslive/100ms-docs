...
3. **Is live translation also supported?**
    ...
    Yes. You can enable per-role language translation for live captions by configuring the `translation` field in the [transcription destinations API](/server-side/v2/policy/create-template-via-api#transcriptions---translation-object). Each role can receive captions translated into their configured language.
