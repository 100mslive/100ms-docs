...
<h2> sample JSON: object &lt;&gt; sub-objects mapping </h2>
...
## Templates configuration - Dashboard
...
| roles        | Array of [roles objects](#roles-object) with a collection of arguments and objects such as `name`, `publishParams`, `subscribeParams`, `permissions`, `priority`, `maxPeerCount`, etc. |
| settings     | Object of type [settings](#settings-object) with the collection of arguments and objects such as `region`, `subscribeDegradation`, `recording`, `roomState`, etc.                      |
...
| Argument        | Description                                                                                                                                                   |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| name            | Alias for the role associated with a particular template.                                                                                                     |
| publishParams   | Object of type [publishParams](#publish-strategy) with the collection of objects such as `allowed`, `audio`, `video`, `screen`, etc                           |
| subscribeParams | Object of type [subscribeParams](#subscribe-strategy) with a mix of arguments and objects like `subscribeToRoles`, `maxSubsBitRate`, & `subscribeDegradation` |
| permissions     | Object of type [permission](#permissions) with collection of objects like `browserRecordings`, `rtmpDestinations`, & `hlsDestinations`.                       |
| priority        | Priority will determine the order in which the roles will be degraded. A lower number represents a higher priority.                                           |
