...
| maxPeerCount    | `int`    | The number of peers allowed for a particular role in a session. <br/><br/> `Default`: 0 (unlimited). <br/><br/> `Allowed values`:<br/><ul><li>**-1** : no peer will be allowed to join with that role.</li><li>**0** : unlimited.</li><li>**30** : only 30 peers are allowed to join with that role (you can set a different custom limit as well).</li></ul> | No       |
