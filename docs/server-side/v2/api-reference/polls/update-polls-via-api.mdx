---
title: Update polls via API
nav: 3.84
---

This endpoint is used to update polls. A recommended way is to provide a list of questions while creating poll, then update those questions by modifying them using update/delete question/option API.
...
```bash
curl --location --request POST 'https://api.100ms.live/v2/polls/<poll_id>' \
