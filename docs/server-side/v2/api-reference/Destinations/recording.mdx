...
Size of the recordings are a function of the video and audio bitrate configured for publishing and the duration of the peer in the session.
For one hour of recording, the file size can be estimated as:
    > size = video + audio bitrate (in kbps) * 0.45 MB/hour
    > Example: Size/hour of a recording with 360 4:3 video publish at 300kbps would be
...
![Video Publish Bitrate](/docs/v2/RolePublishSetting.png)
