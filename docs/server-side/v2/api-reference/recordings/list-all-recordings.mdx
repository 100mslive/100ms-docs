...
A common scenario is to fetch the recording assets for a room or session. To do so, follow these steps

1. Get the recording job with the `room_id` or `session_id` query parameter on the list API shown below.
2. Get the recording job `id` from the filtered response.
3. Use the [get recording API](./get-recording) with the above `id` to get assets for the recording job.
