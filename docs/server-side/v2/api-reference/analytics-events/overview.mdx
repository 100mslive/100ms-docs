...
| `events`  | List of events, refer individual event page for details (Eg: [Track event object]((/server-side/v2/api-reference/analytics-events/track-events#track-event-object)))                     |

## Pagination
- Results are paginated with default events count `10`, extendable upto `100` by specifying it in `limit` query parameter.
