...
Kind of track (e.g. audio, video, screen) can be determined by `type` and `source` parameter in the [track event object](/server-side/v2/api-reference/analytics-events/track-events#track-event-object).
