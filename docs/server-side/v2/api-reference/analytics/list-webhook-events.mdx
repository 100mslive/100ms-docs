...
Retrieves webhook delivery history, including both successful and failed webhook deliveries. You can use this API to debug webhook delivery issues and monitor webhook health.

<GetRequest title="https://api.100ms.live/v2/analytics/events/webhooks" />
<Request id="req-0">

```bash
curl --location --request GET \
    'https://api.100ms.live/v2/analytics/events/webhooks?start_time=2025-12-28T00:00:00Z&status=failed&limit=20' \
...
| Endpoint        | Limit                              |
| --------------- | ---------------------------------- |
| `GET /webhooks` | 5 requests per minute              |
