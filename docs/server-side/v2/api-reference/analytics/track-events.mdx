...
for example when the user has unmuted audio, shared screen, or stopped the video. Every event has a `mute` property to determine mute status at the time of the event.
...
Kind of a track (for example audio, video, video screenshare) can be determined by `type` and `source` parameter in the [track event object](/server-side/v2/api-reference/analytics/track-events#track-event-object).
...
| ---------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| type       | `string` | Type of events to be fetched (valid values `track.add.success`, `track.update.success`, `track.remove.success`) <br /><br /> Can be specified multiple times to include more than one event (for example `type=track.add.success&` `type=track.update.success`) | yes      |
...
| joined_at  | `timestamp (in UTC)` | Timestamp when the peer joined the session in RFC3339 format for example `2022-02-09T05:53:23.375Z`                       |
...
| started_at | `timestamp (in UTC)` | Track start time in RFC3339 format for example `2022-02-09T05:53:23.375Z`.                                                |
| stopped_at | `timestamp (in UTC)` | Track stop time in RFC3339 format for example `2022-02-09T05:53:23.375Z` (only specified in `track.remove.success` event) |
