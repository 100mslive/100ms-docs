...
- To get the active duration of a peer, you can pass `peer_id`/`user_id` in the query parameter while fetching events, this would limit events only to specified peer/user.
    ...
    - Find the sum of duration starting from the event with `mute: false` (Eunmute<sub>timestamp</sub>) to the next event with `mute: true`(Emute<sub>timestamp</sub>) until we reach `track.remove.success`.  <br />(i.e active duration: Σ Emute<sub>timestamp</sub> - Eunmute<sub>timestamp</sub>)
