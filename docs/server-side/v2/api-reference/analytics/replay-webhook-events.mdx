...
Initiates a replay of webhooks that failed to deliver or reprocess successful webhooks. This is particularly useful when recovering from server downtime or when you need to reprocess events with updated logic.

## Replay Webhooks

<PostRequest title="https://api.100ms.live/v2/analytics/events/webhooks/replay" />
<Request id="req-0">
...
> **Note:** Maximum 50,000 webhooks per replay request. Duplicate requests with same filters will return existing workflow.
...
1. Starts a workflow that fetches webhooks from the database in batches of 100
2. Each webhook is sent to your configured webhook URL with an `is_replay: true` flag
3. Rate limits are applied to prevent overwhelming your endpoint. Typically, webhooks are sent in batches of 100 events with a few seconds timeout between eaach batch.
Please ensure that your webhook endpoints are scaled up before triggering the replay API
...
| Endpoint                 | Limit                             |
| ------------------------ | --------------------------------- |
| `POST /webhooks/replay`  | 1 request per minute              |
...
- Replay requests with identical filters will automatically reuse the existing workflow
- Workflow IDs are deterministically generated based on the request filters
- Active workflows will return an `already_running` status for duplicate requests
...
<GetRequest title="https://api.100ms.live/v2/analytics/events/webhooks/replay/{workflow_id}" />
<Request id="req-1">

```bash
curl --location --request GET \
    'https://api.100ms.live/v2/analytics/events/webhooks/replay/webhook-replay-customer123-a1b2c3d4' \
    --header 'Authorization: Bearer <management_token>'
```
...
- **Check status of replay webhook workflow :** Depending on the number of webhook events, a replay workflow trigger can take considerable time to complete. This API can be used to check real time status of any running webhook replay workflow and ensure that it's completed successfully.
