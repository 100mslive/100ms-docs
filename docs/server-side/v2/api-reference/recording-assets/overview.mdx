...
* `room-composite`: This asset type is a single video file (mp4) that composes audio/video for all peers in the room. The composition is higher quality (similar to the perspective of a peer in the room) than `room-composite-legacy` ([see detailed difference](../../../../get-started/v2/get-started/features/recordings/overview)).
