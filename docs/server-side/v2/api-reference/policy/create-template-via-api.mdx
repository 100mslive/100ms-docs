...
| autoStart       | `boolean`| Indicates whether this destination should be used while starting recordings automatically on room join  | No       |
...
| enableZipUpload    | `boolean` | By default, the value is 'false'. You can set this to 'true' to enable zipped upload of the HLS VOD asset if the recording storage is your personal storage bucket and `upload` [object](#upload-object) is configured. Setting it to 'true' with the configured storage as 100ms will give an error.                                                                                                                                              | No       |
