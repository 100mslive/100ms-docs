...
| role             | `string` | Indicates the role use to transcribe. Call will be transcribed with the perspective of a peer having mentioned role                                                 | Yes      |
| outputModes      | `array`  | Array of ouput modes for transcript. Valid values: [ `txt`, `srt`, `json` ] (`txt`: Plain text, `srt`: SubRip text, `json`: Granular timestamped word and sentences | No       |
| customVocabulary | `array`  | Array of custom vocabulary words to be used for transcription. This can consist of non-dictionary words like names, slang, abbreviations and domain specific words  | No       |
| summary          | `object` | Object of type `summary`. This can be used to enabled and configure summary                                                                                                     | No       |
...
| context     | `string`  | Provide context of the meetings to help AI get holistic understand about the call. Enabling it to generate better summary (limit: 300 characters)                | No       |
| temperature | `float`   | Temperature determines creativity. Higher values makes AI be more creative, lower values makes it more deterministic and predictable. (default: 0.5) | No       |
| sections    | `array`   | Sections has list of objects. Refers to sections in summary like `Agenda`, `Short Summary`, `Questions raised by participants` (max count: 6)        | No       |

#### transcriptions - summary - sections object

| Name   | Type     | Description                                                                | Required |
| ------ | -------- | -------------------------------------------------------------------------- | -------- |
| title  | `string` | Title of the section eg. `Agenda`, `Short Summary`  (limit: 100 character) | Yes      |
| format | `string` | Format of section. Valid values: [`bullets`, `paragraph`]                  | Yes      |
