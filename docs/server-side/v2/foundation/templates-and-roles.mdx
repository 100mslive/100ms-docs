...
| Name   | Type    | Description                                                        | Required |
| :----- | :------ | :----------------------------------------------------------------- | :------- |
| height | `int`   | Max height of the video track.<br/>Range: 90-720<br/>Default: 180  | No       |
| width  | `int`   | Max width of the video track.<br/>Range: 160-1280<br/>Default: 320 | No       |
...
| Name   | Type     | Description                                                          | Required |
| :----- | :------- | :------------------------------------------------------------------- | :------- |
| region | `string` | Region in which the room will be hosted by default ["in"/"us"/"eu"]. | No       |
...
```bash
curl https://prod-policy.100ms.live/api/v1/templates/<id>/settings -H 'Authorization: Bearer <management_token>' -X POST -H 'Content-Type: application/json' -d '{ "region": "us" }'
```
...
<DeleteRequest
    ...
    desp="This endpoint is used to delete a template. Instead of an id URL parameter name can be provided. Subsequent delete requests on non-existing templates gives 404."
    url="https://prod-policy.100ms.live/api/v1/templates?id=<templateID"
/>
