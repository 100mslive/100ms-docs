...
### role.change.failure
This event will be sent when a peer fails to change role. This can occur when,

1. Current role is same as old role
...
| user_name          | `string`    | User name of the user                          | Test user                            |
| user_data          | `string`    | User data of the user                          | {"isHandRaised":true}                |
| role               | `string`    | Old role of the user                           | host                                 |
| joined_at          | `timestamp` | Timestamp when user joined                     | 2020-11-11T16:32:17Z                 |
| session_started_at | `timestamp` | Timestamp when session started                 | 2020-11-11T16:32:17Z                 |
| error_message      | `string`    | Reason for failure                             | current role is same as old role     |
| session_id         | `string`    | 100ms assigned id to identify the session      | 5f9edc6bd238215aec7700df             |

#### Role change failure Reason

1. `target role clash with requested roles`
2. `peer not joined`
3. `invalid role`
4. `role does not have required permission`
