...
2. `peer kicked` — if the peer is removed by someone else (host or application server) from an active room using the [Client SDK](/javascript/v2/features/remove-peer) or the [Server API](/server-side/v2/active-rooms/remove-peers).
    - If you remove a peer from an [active room](./../active-rooms/overview) using the [client SDK](/javascript/v2/features/remove-peer) or the [server API](/server-side/v2/active-rooms/remove-peers), you can pass the reason as a message and the `peer.leave.success` event will contain the same in the `message` field. Check below sample event for reference.
