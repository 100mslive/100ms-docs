...
3. `websocket closed`: network issues - 100ms server has closed the WebSocket connection due to abrupt network disconnection from the client side (peer) without any further attempts to reconnect.
4. `ice connection state closed`: network issues - When a peer is disconnected due to network issues, the SDK will retry for 60 seconds and disconnect if there are no attempts to reconnect.
5. `server removed peer`: network issues - You will get this error when a peer is removed by the server as there were no attempts to reconnect for 60 seconds and if the client-side app still sends retries after 60 seconds. This happens rarely.
