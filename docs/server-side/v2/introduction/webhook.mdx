...
3. `websocket closed`: network issues
    - _Example scenario:_
        - If there's an abrupt network disconnection from the client side (peer) without any further attempts to reconnect, 100ms server will close the WebSocket connection.
4. `ice connection state closed`: network issues
    - _Example scenario:_
        - When a peer is disconnected due to network issues, the SDK will retry for 60 seconds and disconnect if there are no attempts to reconnect.
5. `server removed peer`: network issues
    - _Example scenario:_
        - When a peer is removed by the server as there were no attempts to reconnect for 60 seconds and if the client-side app still sends retries after 60 seconds. This happens rarely.
