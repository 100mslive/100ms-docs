...
```bash
curl --location --request GET 'https://prod-in2.100ms.live/v2/sessions?url_param1=param1_value&url_params2=param2_value' -H 'Authorization: Bearer {{management_token}}'
```
...
| before <br/>  | `time`   | Timestamp before (inclusive) which session is created. | No       |
| after <br/>   | `time`   | Timestamp before(inclusive) which session is created.  | No       |

### Derived values.

You can calculate additional information from session data using given information. Redundant information like peer duration/ session duration is not provided since they can be calculated from provided response. You can calculate `peer duration` by subtracting joined_at timestamp from left_at. Similarly you can calculate `session duration` by adding all peers' duration.
