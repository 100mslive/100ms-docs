...
## Create Room using API (Deprecated)
...
```bash
curl https://api.100ms.live/api/v2/rooms -H 'Authorization: Bearer  <management_token>' -X POST -H 'Content-Type: application/json' -d '{"name": "test-room", "description": "This is a test room", "recording": {"enabled": true, "upload": {"type": "s3", "location": "test-bucket", "prefix": "test-prefix", "options": {"region": "ap-south-1"}, "credentials": {"key": "aws-access-key", "secret": "aws-secret-key"}}}}'
```
...
```json
{
    ...
    "customer_id": "60b8e13a6eb86d8101b57354",
    "created_at": "2021-08-08T17:24:25.682362823Z",
    "updated_at": "0001-01-01T00:00:00Z"
}
```
...
| name <br/>        | `string` | A unique identifier you can assign to 100ms rooms. This is `case-insensitive`. Accepted characters are `a-z, A-Z, 0-9, and . - : _`. If not provided, this is generated automatically. <br/><br/> **Note**: <br/><br/> (1) If `create Room` is called with an existing room name, then the corresponding room ID is returned <br/><br/> (2) Providing room name will be helpful if you want to disable the room later (disable room API requires room name) | No       |
| description <br/> | `string` | A string to describe your room's usage. E.g. "9PM English Class Batch 2"                                                                                                                                                                                                                                                                                                                                                                                    | No       |
| template_id <br/> | `string` | [Template](../foundation/templates-and-roles) of the room. You can get template ID from room details section on dashboard                                                                                                                                                                                                                                                                                                                                   | No       |
| recording <br/>   | `object` | Object of type `recording`. This object contains information for enabling recording/setting storage location for recordings.                                                                                                                                                                                                                                                                                                                                | No       |
| region <br/>      | `string` | Region in which you want to create room. <br/><br/> `in` - India <br/><br/> `us` - United States <br/><br/> `eu` - European Union <br/><br/> `auto` Automatic region selection (default, and in case this parameter is not provided)                                                                                                                                                                                                                        | No       |
...
### Error messages in Create Room API

| Message                                                            | Error Code | Description               |
| :----------------------------------------------------------------- | :--------- | :------------------------ |
| Error: accepted characters in name are a-z, A-Z, 0-9, and . - : \_ | 400        | invalid name              |
| Request body contains badly-formed JSON (at position 16)           | 400        | invalid post body         |
| Error: provide either id or name, not both                         | 400        | both name and id provided |
| Error: region not supported                                        | 400        | invalid region            |
| Error: template not found                                          | 400        | invalid template_id       |
| Error: Internal server error                                       | 500        | server error              |

## Disable Room

This API can be used to disable a room from further use. Provide all parameters since this is a POST request.

<PostRequest
    title="Disable Room API"
    desp="This endpoint is used to disable a Room. "
    url="https://api.100ms.live/api/v2/rooms"
/>
...
```bash
curl https://api.100ms.live/api/v2/rooms -H 'Authorization: Bearer  <management_token>' -X POST -H 'Content-Type: application/json' -d '{"name": "test-room", "enabled": false}'
```
...
| Name          | Type   | Description                                                      | Required |
| :------------ | :----- | :--------------------------------------------------------------- | :------- |
| enabled <br/> | `bool` | Flag to indicate disabling of the room, should be set to `false` | Yes      |

### URL Parameters

| Name       | Type     | Description                | Required |
| :--------- | :------- | :------------------------- | :------- |
| id <br/>   | `string` | ID of the room if known.   | No       |
| name <br/> | `string` | Name of the room if known. | No       |

## Get Room Data using API

You can query room data using API.

<GetRequest
    title="Get Room API"
    desp="This endpoint is used to get rooms. "
    url="https://api.100ms.live/v2/rooms"
/>
...
```bash
curl --location --request GET 'https://api.100ms.live/v2/rooms?enabled=true' -H 'Authorization: Bearer {{management_token}}'
```
...
```json
{
    ...
    "data": [
        {
            "id": "62153c9d4c24a61350dadbc4",
            "name": "gfxhjh",
            "enabled": true,
            "description": "This is a dummy description",
            "customer_id": "61386d433ab16014b061e9ca",
            "recording": {
                "enabled": false
            },
            "template_id": "615297d65095aca9b1be85ac",
            "region": "in",
            "created_at": "2022-02-22T19:42:21.877Z",
            "updated_at": "0001-01-01T00:00:00Z"
        },
        {
            "id": "621513d24c24a61350dadbc3",
            "name": "be03d295-480f-440d-871f-6ec5961d6f31",
            "enabled": true,
            "description": "This is a dummy description",
            "customer_id": "6202498dfee9f9c2b7e0ecf3",
            "recording": {
                "enabled": false
            },
            "template_id": "621513d136e26606b78f9c44",
            "region": "in",
            "created_at": "2022-02-22T16:48:18.055Z",
            "updated_at": "0001-01-01T00:00:00Z"
        }
    ],
...
| id <br/>      | `string` | ID of the room if known.                                                                                          | No       |
| name <br/>    | `string` | Name of the room if known.                                                                                        | No       |
| enabled <br/> | `bool`   | Room is disabled or not.                                                                                          | No       |
| before <br/>  | `time`   | Timestamp before(inclusive) which room is created in RFC3339 format e.g. `2022-02-09T05:53:23.375Z`.              | No       |
| after <br/>   | `time`   | Timestamp after(inclusive) which room is created in RFC3339 format e.g. `2022-02-09T05:53:23.375Z`.               | No       |
| limit <br/>   | `int`    | Number of rooms returned by response. Range `10`-`20`                                                             | No       |
| start <br/>   | `string` | ID of the room after which to start the response. Rooms are returned in decreasing IDs or `created_at` timestamp. | No       |

### Response

| Name        | Type       | Description                                                                                                                | Required |
| :---------- | :--------- | :------------------------------------------------------------------------------------------------------------------------- | :------- |
| limit <br/> | `int`      | Number of rooms the API tried to find. If returned array has lesser number of results, only those number were found.       | No       |
| data <br/>  | `array`    | Array of rooms or `null` if no room was found.                                                                             | No       |
| last <br/>  | `ObjectID` | Rooms are returned in decreasing IDs or `created_at` timestamp. Last represent the earliest room returned by the response. | No       |

## Postman Collection

<DownloadCollection type="rooms_new" />
