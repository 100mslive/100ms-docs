...
This API can be used to end a currently running room

<PostRequest
    title="End Room API"
    desp="This endpoint is used to end a room. For US/EU rooms use prod-us2/prod-eu2 instead of prod-in2"
    url="https://prod-in2.100ms.live/islb/api/v2/room/end"
/>

<Tabs items={['Request', 'Response']} id="end" />

<Tab id="end-0">

```bash
curl https://prod-in2.100ms.live/islb/api/v2/room/end -H 'Authorization: Bearer  <management_token>' -X POST -H 'Content-Type: application/json' -d '{"id": "61101349d0f56e684b608c08", "reason": "meeting over"}'
```
...
<Response res="200: OK" resText="session is ending" />
...
| Name         | Type     | Description                         | Required |
| :----------- | :------- | :---------------------------------- | :------- |
| id <br/>     | `string` | Room id of the room to be ended     | Yes      |
| reason <br/> | `string` | Reason for ending the room          | Yes      |
