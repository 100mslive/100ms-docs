...
1. If `peer_id` is defined -- message will be sent only to the specified connected peer.
2. If the `role` is defined -- message will be sent to all the connected peers assigned with that particular role.
3. If both `peer_id` and `role` are defined -- preference will be given to the peer_id and the message will be sent only to the specified connected peer.
4. If both `peer_id` and `role` are not defined -- message(broadcast) will be sent to all the connected peers.
...
-   For example, Suppose a teacher cannot attend a virtual classroom at the last minute. In that case, you can inform all the connected peers(students) by sending a broadcast message to inform the teacher's unavailability.
