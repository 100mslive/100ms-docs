...
A composite recording is different from track-level recordings: single file per track (audio, video). To use track-level recordings instead of composite, enable [SFU recordings](../../api-reference/legacy-api-v1/destinations/recording) on your template.
