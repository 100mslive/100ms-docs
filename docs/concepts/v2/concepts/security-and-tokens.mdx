...
-   [Management token](#management-token): Authenticate the requests to the **100ms server-side REST API**

Both of these tokens have an expiry.
...
-   `app_access_key` and `app_secret` from the [dashboard](https://dashboard.100ms.live/developer).
...
-   **user_id**: This identifier can be used to map a 100ms peer to your own internal user object for business logic. Specify your internal user identifier as the peer's user_id.
...
#### Code sample: Generate auth token
