...
100ms _client-side SDKs_ use Auth Tokens to authenticate a peer (participant) while [joining a room](/javascript/v2/how--to-guides/set-up-video-conferencing/join). Generate this token on the server side and make it available for your client-side apps that use the 100ms SDKs.
...
If you are building integration with 100ms, you can get auth tokens from the 100ms token endpoint without hosting a token generation backend service. Refer to [this guide](/javascript/v2/get-started/token-endpoint#get-an-auth-token-using-token-endpoint) for more information.

#### Get auth tokens from the dashboard

If you are building your first app by following one of our [quickstart guides](/javascript/v2/get-started/javascript-quickstart), you can get the auth token directly from 100ms dashboard to join a room for the first time. Refer to [this guide](/javascript/v2/guides/token#get-a-temporary-token-from-100ms-dashboard) for more information.
