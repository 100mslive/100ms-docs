...
3. Via **[Room Codes](/concepts/v2/concepts/rooms/room-codes/room-codes)** - Simply get auth tokens by generating room codes using server-side [Create Room Code API](/server-side/v2/api-reference/room-codes/create-room-code-api) . This approach eliminates the need to set up an auth token server at your end. Head to [Room Code Authentication](/concepts/v2/concepts/rooms/room-codes/room-code-auth) for more details.
