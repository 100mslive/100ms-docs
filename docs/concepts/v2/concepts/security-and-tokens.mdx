...
just say that your app would call your server first to get the token. This is so that the secret is not exposed on client side. The token is then passed to 100ms SDK for connecting.
As a small note we can add that ideally you would want to tie it with your user auth.
