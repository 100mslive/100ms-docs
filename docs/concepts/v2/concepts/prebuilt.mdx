...
2. Create Room Link structure : Append `template-subdomain` and `room code` to the room link format.
   ...
   Here is an sample pseudo code snippet involving creating a room and then creating room links for that room.
...
<FlexContainer>
<Callout icon="link" title="iFrame integration benefits">
