...
1. **Create Room Codes** - Room codes can be made available in two ways:
...
2. **Retrieve role specific room code** - you can get the room code in two ways:
...
3. Once you have a role specific room code, call the SDK function `getAuthTokenByRoomCode` with the retrieved room code. This function retrieves the auth token that is required to join the room.
...
Refer our Quickstart Guides to get started with room code based authentication:
