...
1. If the web app does not load correctly, it retries up to 3 times
    ...
    * To fulfill this condition, your app must join the 100ms room successfully **within 10 secs** of page load
1. Auto-stop when beam is the only one in the room
    * If all peers have left the room and there are no re-joins within 5 mins, recording stops automatically 
