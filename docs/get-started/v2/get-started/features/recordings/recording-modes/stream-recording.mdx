...
A stream recording combines the individual video and audio tracks for each pariticipant into a single recording file. If the peer publishes his desktop, the screen share video track is generated as an additional stream recording asset.
Stream Recording Assets are classifed into three types:
    ...
    **Audio** - contains only the audio track from micrphone input for audio only calls where video tracks were not published at all by the peer.
...
1. Using **[List Recording Assets API]**(/server-side/v2/aoi-reference/recording-assets/list-all-assets) get the all stream recording assets for a session_id and asset_type=`stream`
