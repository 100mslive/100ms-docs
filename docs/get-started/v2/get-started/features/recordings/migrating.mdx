...
> **Note**
>
> Another way to start SFU recording is to programmatically create a room using the [Create Room REST API](/server-side/v2/api-reference/Rooms/create-via-api) and passing `recording_info` argument as `enabled`. Instead, we recommend to use the [Room APIs](/server-side/v2/api-reference/Rooms/overview) paired with the [Recording APIs](/server-side/v2/api-reference/recordings/overview) if you wish to programmatically control room and recording operations.
