...
You can store your recordings on a cloud storage provider through the recording settings of your template. We support:

- Amazon Simple Storage Service (AWS S3)
- Google Cloud Storage (GCP)
- Alibaba Object Storage (OSS)

If you haven't configured a cloud storage service, then a recording will be stored temporarily (for 15 days) in a 100ms bucket. Recording links can be accessed on the [100ms dashboard](https://dashboard.100ms.live/sessions) or [through the API](/server-side/v2/api-reference/recording-assets/get-asset).

### Configure with 100ms Dashboard
