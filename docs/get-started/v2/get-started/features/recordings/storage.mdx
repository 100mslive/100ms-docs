...
You can configure storage in your template's recording settings on the 100ms Dashboard. As an example, this is how you would configure a GCP location:
...
2. Go to 100ms Dashboard and go to template configuration by selecting **'Configure'** under **Templates**

3. Head over to the **Recordings** tab.
...
The following video will walk you through the above steps.
...
- Live Stream Recording (HLS): [hls.recording.success](/server-side/v2/introduction/webhook#hlsrecordingsuccess)
...
4. VOD recording (Zipped - available in HLS): `s3://<location>/<prefix>/hls/<room_id>/<start_date>/<epoch>/vod/Rec-<room_id>-<epoch>.zip`
