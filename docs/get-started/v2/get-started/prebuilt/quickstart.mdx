...
### Set up a template
A [template](/v2/get-started/concepts/templates-and-roles) governs how its Prebuilt UI functions. Create a template using dashboard or create [template via API](/v2/api-reference/policy/create-template-via-api).

### Create a room
When you create a template on 100ms dashboard, a room gets created by default. You can programatically create rooms using [create room API](/v2/api-reference/Rooms/create-via-api).
...
- via API - Use [Create Room Codes API](/v2/api-reference/room-codes/create-room-code-api) to create room codes and get room codes using [retrieve room codes API](/v2/api-reference/room-codes/get-room-code-api).
...
<FlexContainer>
  <Callout icon="link" title="Prebuilt Link for roles">
    - Host Prebuilt Link :
    https://`template-subdomain`.app.100ms.live/meeting/`room-code-for-host`<br/>
