...
### Set up a template
A [template](/get-started/v2/get-started/concepts/templates-and-roles) governs how its Prebuilt UI functions. Create a template using dashboard or create [template via API](/server-side/v2/api-reference/policy/create-template-via-api).

### Create a room
When you create a template on 100ms dashboard, a room gets created by default. You can programatically create rooms using [create room API](/server-side/v2/api-reference/Rooms/create-via-api).
