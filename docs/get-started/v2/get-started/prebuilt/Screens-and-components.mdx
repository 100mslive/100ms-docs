...
As Prebuilt UI maps to its template configuration, you'll be able to preview and customize Prebuilt behavior for each role in your template. This guide explains how Prebuilt's default can be customized for its role using screens and components.
...
### Room Screen
Prebuilt's room screen provides components that enables interactivity an within a room. Select a role to start customizing.
...
##### Enable Overlay View
Enabling a chat overlay view. (Default state: `false`)
This view is only available on mobile views. Not supported on large screen apps.

##### Allow Pinning messages
Enabling this allows the selected role to pin chat messages in Prebuilt.  
...
Defining a role as prominent for others (or self) defines the layout to be fixed throughout the session and cannot be altered during the session. This is best suited for scenarios where a specific role remains prominent throughout the session for example webinars, livestreaming; whereas Spotlighting self tile or others tile helps make certain peer tiles to be prominent within the session temporarily allowing more flexible layout changes in a session.
