...
<Callout icon="link" title="Room Code Authentication Basics">
    ...
    token](/concepts/v2/concepts/Authentication/AuthToken) at your end to join a room. - If you're
    using 100ms [Prebuilt](https://www.100ms.live/docs/concepts/v2/concepts/prebuilt) solution,
    Prebuilt inherently takes care of generating the auth token using room codes. - When building a
    custom UI, you need to make an additional SDK API call `getAuthTokenByRoomCode` to generate the
    auth token.
</Callout>
