...
- **Room codes are recommended** for creating shareable join links for sessions. They can also power authenticattion for peers joining a session, eliminating the need to generate an authentication token.
...
| --- | --- | --- |
| Parity | 10 character short codes; unique to a `role` and `room_id` combination. Additional user context can be passed to make these codes user specific. | JWT token. Auth token is unique to a `role`, `user_id` and `room_id`. |
