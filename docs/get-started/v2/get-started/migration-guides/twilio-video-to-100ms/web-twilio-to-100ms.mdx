...
Replace Twilio's **`Video`** object with 100ms' `hmsStore` and  `hmsActions`.
...
You can optionally pass any user `metadata` that you might require in your setup. 
...
const hmsActions = new HMSReactiveStore().getActions();
...
### Twilio: Rendering Local and Remote Video
...
### 100ms: Rendering Local and Remote Video

100ms SDK abstracts away the direct handling of media tracks, providing a more straightforward approach.

**Rendering Local Video:**

100ms automatically handles local video tracks when you join a room. However, you need to manage the rendering part.
...
import { HMSReactiveStore, selectLocalPeer } from "@100mslive/hms-video-store";

const hmsStore = new HMSReactiveStore().getStore();
...
hmsStore.subscribe(() => {
    ...
    if (localPeer && localPeer.videoTrack) {
        ...
        localVideoElement.srcObject = new MediaStream([localPeer.videoTrack.track]);
    }
}, selectLocalPeer);
```
...
Similar to local video, you subscribe to remote peers' video tracks.
...
function renderRemoteVideos() {
    ...
    peers.forEach(peer => {
        if (!peer.isLocal && peer.videoTrack) {
            ...
            remoteVideoElement.srcObject = new MediaStream([peer.videoTrack.track]);
            remoteVideosContainer.appendChild(remoteVideoElement);
        }
    });
}

hmsStore.subscribe(renderRemoteVideos, selectPeers);
```
