...
> ⚠️ It's important to make sure the object is serializeable if not primitive. String, Boolean etc are fine to pass and read with casting. For a custom class, do add `Gson` `Serializable` annotations or otherwise ensure that the object can be recreated after being sent across the network. `Keep` annotations would also work and might possibly be the easiest.
...
sessionStore.get("mykey", object : HMSSessionMetadataListener {
    override fun onSuccess(sessionMetadata: Any?) {
        // Use value
        val data = sessionMetadata as CustomMetadata
    }
...
sessionStore.get("mykey",new  HMSSessionMetadataListener(){
    ...
    public void onSuccess(@Nullable Object sessionMetadata) {
        // Use value
        CustomMetadata data = (CustomMetadata) sessionMetadata
    }
