...
```
2. Toggle noise cancellation on in your application with `hmsSDK.enableNoiseCancellation(true, hmsActionResultListener)` in your `onJoin` callback.
...
```kotlin
hmsSDK.setNoiseCancellationEnabled(true, object : HMSActionResultListener {
    override fun onError(error : HmsException) { 
        // handle errors
    }

    override fun onSuccess() {
        // the value was set as expected
    }
}) // or false
```
> Note: Do handle the `onError`, if noise cancellation is disabled from your dashboard, the mobile app software won't be able to turn it on.
