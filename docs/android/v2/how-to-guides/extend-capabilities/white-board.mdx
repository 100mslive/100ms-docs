...
## Starting whiteboard
To start a whiteboard make sure the role has appropriate permission and the current whiteboard is not open.
...
If the `isOwner` is true in `HMSWhiteboard` model then user has to refresh and close the `WebiView`
