...
## v2.9.87 - 2026-09-15
### Fixed
* Fixed joins taking a fixed ~11 seconds, or a multiple of it, on networks where the IPv6 route to our servers silently drops traffic. The SDK now checks which address family is actually reachable before connecting and uses that one, instead of waiting out a connection timeout on an unreachable address. Networks with working IPv6 continue to prefer it.

### Changed
* Reduced the connection timeout from 11 to 5 seconds, so an unreachable address is given up on sooner.
