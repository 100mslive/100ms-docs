...
### Changed
- `HMSChangeTrackStateRequest` is now sent even when peers are muted. Previously, if a remote peer requested a peer to mute, there would be no ChangeTrackStateRequest. This request can safely be ignored because the SDK already muted the peer but may be helpful in recognizing that the mute came externally.
- The way packet loss is measured for track degradation was changed.
- HMSPeer's toString now includes `customerId` and `customerDescription`.
