...
Please note that for a peer to share their screen, their role must have screenshare enabled in the dashboard.
...
To start screen share, app needs to call the `startScreenshare` method of `HMSSDK`, which takes in two parameters -
    The first one is `HMSActionResultListener` which is the callback object needed to inform about `success`
        or `failure`.
...
Application needs to call the `stopScreenshare` method of `HMSSDK` with an instance of `HMSActionResultListener`
...
Note :-
    Stop screenshare shuold also be called from `onDestroy()` of the `activity`/ `fragment` that is
