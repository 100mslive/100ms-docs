...
1. *message*: The String message you want to show people when they are forced to leave. This is a good place to thank them for joining the call and informing them that it has ended.
...
> 💡 After calling endRoom the video calling UI should be disposed of as well since only the other peers will get the `onPeerRemoved` callback. The caller has to rely on the `onSuccess` callback for `endRoom` to decide when to terminate the call UI locally.
