...
> 💡 Note: Gathering stats takes several ms and can be a large operation if requested on a room with thousands of peers. Consider whether the addtional load is necessary.
