...
```java
private boolean hasEndRoomPermissions(HMSSDK hmsSdk) {
...
> 💡 This is the same callback that will be triggered if a peer was [removed from a room](RemovePeer) as well. Except that `roomEnded` will be true when the entire room was ended.
