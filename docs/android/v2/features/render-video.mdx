...
### Capture Video Frame
You can get a high quality bitmap object of the frame getting rendered on the HMSVideoView

```kotlin
hmsVideoView.captureBitmap(onBitmap: (Bitmap?) -> Unit, scale: Float = 1.0f)
```
Incase you are capturing video frame for the local peer's camera, remember to disable `auto re-size` option in `HMSVideoTrackSettings` before doing this as described [here](https://www.100ms.live/docs/android/v2/advanced-features/track-settings#disableautoresize).
This would disable publish degradation, so it wont effect the quality of the image based on your upload network bandwidth

### Disabling Auto Simulcast

HMSVideoView has an automatic simulcast layer selection capability which is enabled if adaptive bitrate is enabled. You can check more about it [here](https://www.100ms.live/docs/android/v2/foundation/adaptive-bitrate) 
