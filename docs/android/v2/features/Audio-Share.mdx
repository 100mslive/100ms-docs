...
`hmssdk.startAudioShare` also takes an optional third parameter of type notification. See how to build a custom notification in the android docs [here](https://developer.android.com/training/notify-user/build-notification).
...
Note that the `TALK_ONLY` mode is equivalent to regular mode, that is without starting this API
