...
If you find that a certain device echos when it joins a meeting despite this, turning off its hardware echo cancellation may solve the problem. This is because if the SDK detects hardware echo cancellation is supported by the device, it prefers hardware over software, even if the hardware happens to be faulty.
