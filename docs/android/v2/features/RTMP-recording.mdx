...
Here are the relevant properties inside the `HMSRoom` object which you can read to get the current recording/streaming status of the room namely: `rtmpHMSRtmpStreamingState`, `browserRecordingState`, `serverRecordingState`, `hlsStreamingState`.
...
Error is of the following form and may contain an error code or message if one occured.

```kotlin
data class RtmpError(
  val code : String?,
  val message: String?
)
```
...
Error is of the following type and may contain an error code or message if present.
