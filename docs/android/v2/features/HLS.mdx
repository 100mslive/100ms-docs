...
Optionally you may specify an `HMSHlsRecordingConfig` within the `HMSHLSConfig`.
...
1. `singleFilePerLayer` requests individual recordings for supported recording quality levels. Without this, only the highest quality recording will be made available. Default is false so only the highest quality recording will be preserved.
2. `videoOnDemand` will turn on video on demand streaming, if false will live stream instead. Default is false.
