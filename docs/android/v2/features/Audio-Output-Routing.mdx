...
Audio Routing can be helpful in the case when user wants to switch output to device other than the default focussed one by OS.
...
```kotlin
fun setAudioDeviceChangeListener(audioManageDeviceChangeListener: AudioManagerEvents)
```
add a listener to get updates whenever a device is removed or attached to the device.
`audioManageDeviceChangeListener` -  listner to be passed to be able to listen to device updates.
...
```kotlin
        hmsSDK.setAudioDeviceChangeListener(AudioDevice.SPEAKER_PHONE)
```
</Tab>
