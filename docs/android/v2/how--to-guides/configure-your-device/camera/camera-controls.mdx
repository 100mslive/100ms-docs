...
Takes the highest quality picture from the local video stream in `jpg` irrespective of publisher's resolution.
...
> 🔑 Note: To get correct orientation of the captured image, ensure the image extension is correct while saving the file.
