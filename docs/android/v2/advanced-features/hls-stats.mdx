...
dependencies {
 // See the latest version in the jitpack badge above.
  ...
  implementation 'com.github.100mslive.android-sdk:player-stats:2.2.8'
    }
```
...
| video width | The width of the video  | px | Used to know the resolution being played |
| video height | The height of the video | px | Used to know the resolution being played |
| video frameRate | The frame rate | frames per second | Used to know the FPS |
| droppedFrames | The number of dropped frames since the last call to this method | Int | Used to calculate the total num of dropped frames  |
| dropped frames elapsedMs | The duration over which the frames were dropped since the last callback | ms | Used to calculate rate at which frames are being dropped |