...
3. [How to set a peer's metadata.](#setting-metadata)
...
The data may be any arbitrary string, it cannot be changed to null.
...
Whenever a remote peer's metadata is updated a callback will be received in `onPeerUpdate(update : HMSPeerUpdate, peer : HMSPeer)` of `HMSUpdateListener` where the `update` will be of type `HMSPeerUpdate.METADATA_CHANGED`.
