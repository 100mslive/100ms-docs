...
Min api version for the virtual background plugin is 21. Same as the min api version core sdk for 100ms. The lowest SDK version it can work with is REPLACE_THIS. It is built for armeabi-v7a, arm64-v8a, x86, and x86_64 architectures.
...
That's it. You have instanstiated virtual background plugin successfully. 🥳
...
`HMSVirtualBackground.isSupported` can be used to check if the API level of device is supported or not.
...
Image will be fit to the video by maintaining the aspect ratio.
