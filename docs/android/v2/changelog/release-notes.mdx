...
1. Allow echo cancellation, noise suppression and automatic gain control to be configureable from `HMSAudioTrackSettings`.
2. Limit the number of peers in the room with [Max Peer Count](https://www.100ms.live/docs/javascript/v2/how--to-guides/debugging/faq#is-there-any-way-to-limit-participants-in-a-room).
3. Added extra analytics for 100ms. (cluster)
4. Fixed Remote peer’s HMSRole having empty permissions, publish params and subscribe params.
5. Improved manual focus for the camera.
7. Pixel 3 phones now always use software decoders to resolve an issue that happens when they don't.
8. Send screenshare track info separately in local statistics.
