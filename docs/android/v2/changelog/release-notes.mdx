...
### Fix
- SessionStore API fix: While it can still take `Any` datatypes, it must return `JsonElement` because the SDK can't parse the values back. Do see changes to the [doc](../how-to-guides/interact-with-room/room/session-store.mdx)
