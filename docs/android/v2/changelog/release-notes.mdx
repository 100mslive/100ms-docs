...
### Breaking Changes
- Joining the room via [Short Code](https://www.100ms.live/docs/concepts/v2/concepts/rooms/room-codes/room-codes) is simplified with the new `getAuthTokenByRoomCode` API. Here's how the android flow is [streamlined](https://www.100ms.live/docs/android/v2/get-started/quickstart#get-auth-token). It was moved from external to internal to the SDK.
- `customerUserID` is now nullable for both local and remote peers.
