...
Note that peers with no video tracks will never have onTrackUpdate called with a video (so they should be watched for in onPeerUpdate) and peers with video tracks should be watched for in `onTrackUpdate` since tracks will come a few milliseconds after the the first time a peer appears in onPeerUpdate.
