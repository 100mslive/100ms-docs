...
To join a video call you need an authentication `token` and a `room id`. You can generate token on the sdk using `TokenUtils.getAuthTokenByRoomCode()` method 

## Minimum Requirements

-   SDK version 2.5.8 or higher

```kotlin section=JoinRoom sectionIndex=1
func generateToken() {
   TokenUtils.getAuthTokenByRoomCode(TokenRequest(#room-code, #user-id), null , object :
      HMSTokenListener {
