...
Sometimes during calls, we need to deafen or turn up a particular peers' volume, for example, Suppose a remote peer is too loud compared to others, you may want reduce their volume level and vice-versa.

100ms SDK gives you option to adjust volume of a remote peer. The change in volume level is only effective locally, remote peers remains unaffected.

You can use `setVolume` method available on `HMSSDK` instance to change volume levels of a remote peer. `setVolume` method accepts 2 parameters as follows -
