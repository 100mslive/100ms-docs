...
When available is available, `HMSUpdateListenerActions.ON_PEER_UPDATE` event is emitted with `HMSPeerUpdate.NETWORK_QUALITY_UPDATED` update type. Rest is same as mentioned in [above](#requesting-reading-network-quality-updates) section.
