...
100ms _client-side SDKs_ use App Tokens to authenticate a peer (participant) while [joining a room](/react-native/v2/features/join). Generate this token on the server-side and make it available for your client-side apps that use the 100ms SDKs.
