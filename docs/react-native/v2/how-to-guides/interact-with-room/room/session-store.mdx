...
Session store is only available after user joins the session. To obtain a reference to the session store, you need to override the `ON_SESSION_STORE_AVAILABLE` function of the `HMSUpdateListener` protocol. Once you get the reference to the store you can save it in your app for accessing later.
