...
hmsInstance?.addEventListener(
    ...
    onPeerListUpdatedHandler,
);

const onPeerListUpdatedHandler = ({
  addedPeers,
  removedPeers,
}: {
  addedPeers: HMSPeer[];
  removedPeers: HMSPeer[];
    ...
    // In Large Room, `ON_PEER_LIST_UPDATED` event is emitted
    // with list of all the peers present in the room.

    // You can subscribe to `ON_PEER_LIST_UPDATED` event to get list of added or removed peers -
    // `addedPeers` is list of added peers in room
    // `removedPeers` is list of removed peers from room
};
```
