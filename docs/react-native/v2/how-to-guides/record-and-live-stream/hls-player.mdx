...
- Take a look at the [HLS Streaming](./hls) docs about how to start a hls stream in a HMS room.

- [Here's](./hls#current-room-status) how to read the hls url from the room.

From above links, we now know that we can get the HLS stream URL from `ON_JOIN` and `ON_ROOM_UPDATE`
events.
...
- <strong>Play the HLS Stream as soon as `HMSHLSPlayer` component is visible on screen</strong>
  ...
  We can render `HMSHLSPlayer` component and pass hls stream url to it to play HLS stream
...
<PortraitImage
    src="/docs/docs/v2/hls-player-controls-ui-showcase.png"
...
interface HMSHLSPlayerStatsUpdateEventData {
  ...
  // [iOS only] Total duration that the user has watched the stream
  ...
  // [Android only] The frame rate per second (FPS)
  frameRate?: number;
