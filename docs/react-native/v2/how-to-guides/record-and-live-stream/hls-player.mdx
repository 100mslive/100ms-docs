...
`HMSHLSPlayer` component is available in `v1.7.0` and above of `@100mslive/react-native-hms` package.


## How to Start HLS
- Take a look at the [HLS](./hls) section about how to start an hls stream in a HMS room.
- [Here's](./hls#current-room-status) how to read the hls url from the room.

Therefore, you can get the HLS stream URL from `ON_JOIN` and `ON_ROOM_UPDATE` events as mentioned
in the link above.


## How to Play a HLS Stream
...
- Play the HLS Stream as soon as `HMSHLSPlayer` component is visible on screen

   can render `HMSHLSPlayer` component and pass hls stream url to it to play HLS stream
...
- Render `HMSHLSPlayer` component first and Play later
...
> The actual HLS Stream URL looks something like: `"https://cdn.100ms.live/beam/....../master.m3u8"`
...
## How to Hide Video Player UI

By default, Video Player UI or Controls are visible to user.

We can hide Video Player UI or Controls by using `enableControls` prop on `HMSHLSPlayer` component -

```js
<HMSHLSPlayer
  // Pass `false` to `enableControls` prop to hide the Video Player Controls
...
We can seek forward and/or backward on the HLS Playback using the `seekForward` and `seekBackward` methods respectively
available on the `ref` of `HMSHLSPlayer` component.

`seekForward` and `seekBackward` methods accepts number of seconds to seek by as a first parameter.
...
... TODO: some introduction line here ...
...
- Use HLS Stats received from the `HMSHLSPlayer` component
  ...
  // TODO: complete description of stats
  ```js
  type HMSHLSPlayerStatsUpdateEventData = {
    // current network speed of the user
    bandWidthEstimate: number;

    // total Bytes downloaded in a session
