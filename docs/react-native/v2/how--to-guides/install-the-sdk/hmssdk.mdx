...
// Will only create `HMSSDK` instance if we don't have one
if (
  // checking if we already have `HMSSDK` instance
  !(hmsInstanceRef.current instanceof HMSSDK)
) {
  // saving reference to `HMSSDK` instance
  hmsInstanceRef.current = await HMSSDK.build();
}
```

If you want to create new `HMSSDK` instances everytime, then before creating the new instance, you always have to _[destroy](#what-does-destroy-method-do) the previous `HMSSDK` instance._
...
// checking if we already have `HMSSDK` instance
if (hmsInstanceRef.current instanceof HMSSDK) {
  // destroying previous `HMSSDK` instance before creating new instance
  await hmsInstanceRef.current.destroy();

  // clear hms reference from other places
}

// creating new `HMSSDK` instance
hmsInstanceRef.current = await HMSSDK.build();
// set hms reference in other places
```
