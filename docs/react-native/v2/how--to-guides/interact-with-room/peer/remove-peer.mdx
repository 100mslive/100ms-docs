...
> 💡 Note: This is the same event that will be emitted if a peer [ends the room](/docs/react-native/v2/how--to-guides/interact-with-room/room/end-room). Except that `roomEnded` will be `false` when a peer is removed from room.
