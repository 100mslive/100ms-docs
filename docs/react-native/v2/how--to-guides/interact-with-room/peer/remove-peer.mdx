...
Can't let just anyone remove others from the video call room. First, you need to create a [role](/concepts/v2/concepts/templates-and-roles) with the permissions to remove others.
...
You can update your apps' UI and [Free Resources](/react-native/v2/how--to-guides/set-up-video-conferencing/release-resources) on receive of `HMSUpdateListenerActions.ON_REMOVED_FROM_ROOM` event.
...
> 💡 Note: This is the same event that will be emitted if a peer [ends the room](/react-native/v2/how--to-guides/interact-with-room/room/end-room). Except that `roomEnded` will be `false` when a peer is removed from room.
