...
Also, ensure that there's no typo / spelling mis-matches betweeb the App Group enabled on Main App Target & the Screenshare Broadcast Extension Target.

![Same App Group for both Targets](/docs/v2/same-app-group.png)


- Running on Simulator: Starting Screenshare from an Simulator is not supported by Apple. You can start Screenshare only from an actual iOS device like an iPhone or iPad.


- iOS Deployment Target Version: 100ms React Native Package is supported for iOS 12 and above versions. Ensure that the Minimum iOS Deployment Target Version is set to `12.0` or above in your Xcode project & the Podfile.
