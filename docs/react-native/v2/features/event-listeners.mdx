...
Check out the [Event Listener Enums](../event-listeners-enums) docs to understand the Update Types emitted by the SDK for events like `ON_PEER_UPDATE`, `ON_TRACK_UPDATE`, `ON_ROOM_UPDATE`, etc.
