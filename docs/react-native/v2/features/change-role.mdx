...
Now, all peers in the room will receive an `HMSPeerUpdate.ROLE_CHANGED` callback so that they both can do the necessary UI updates. Now the user becomes a speaker and the audio publishing will start automatically.
