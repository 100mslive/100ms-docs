...
```js
// You can get `HMSTrack` object from `HMSPeer` object or `HMSUpdateListenerActions.ON_TRACK_UPDATE` event

const audioMuted = peer.audioTrack?.isMute(); // audioTrack property can be undefined on peer object

const videoMuted = peer.videoTrack?.isMute(); // videoTrack property can be undefined on peer object
```
...
localPeer.localAudioTrack().setMute(!audioMuted); // pass opposite value of mute status to `setMute`

const videoMuted = localPeer.videoTrack?.isMute();

// Toggling Local Video, video will be unmuted
localPeer.localVideoTrack().setMute(!videoMuted); // pass opposite value of mute status to `setMute`
```
