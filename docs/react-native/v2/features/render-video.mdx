...
const onTrackListener = (data: { peer: HMSPeer; track: HMSTrack; type: HMSTrackUpdate }) => {
  ...
  if (data.track.type === HMSTrackType.VIDEO) {

    // If Video track is added, you can use `trackId` to render video
    if (data.type === HMSTrackUpdate.TRACK_ADDED) {
      console.log(`${peer.name}s' video track Added: ${data.track.trackId}`);
      console.log(`Render HMSView with trackId: ${data.track.trackId}`);
    }

    // If Video track is removed, remove `HMSView` which is using this `trackId`
    if (type === HMSTrackUpdate.TRACK_REMOVED) {
      console.log(`${peer.name}s' video track Removed: ${data.track.trackId}`);
      console.log(`Remove HMSView rendering trackId: ${data.track.trackId}`);
    }
    ...
    if (
      ...
      console.log(
        `Update UI to show Muted/Unmuted/Degraded/Restored updates: ${data.track.trackId}`
      );
    }
  }
};
