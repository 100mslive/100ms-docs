...
* [Broadcast messages](#sending-broadcast-messages) are sent to "Everyone" in the chat, using - `hmsInstance.sendBroadcastMessage`.

* [Direct messages](#sending-direct-messages) are sent to a "specific person", using - `hmsInstance.sendDirectMessage`.

* [Group messages](#sending-group-messages) are sent to "everyone with a particular `HMSRole`". Such as all `hosts` or all `teachers` or all `students`, using - `hmsInstance.sendGroupMessage`.
