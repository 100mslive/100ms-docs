...
On Android we receive `isTerminal` property in HMSException - Use this property to determine whether peer can continue in the room or not
...
const onError = (error: HMSException) => {
    ...
    if (Platform.OS === 'android') {
      if (error?.isTerminal === true) {
        // navigate to home screen
      }
    } else {
      if (error?.code === 4005 || error?.code === 2000) {
        // navigate to home screen
      }
    }
};
