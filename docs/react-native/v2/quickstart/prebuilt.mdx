...
3. react-native-vector-icons package
  ...
  ```
  Follow the [official installation steps](https://github.com/oblador/react-native-vector-icons/tree/v9.1.0#installation) for correctly installing React Native Vector Icons `v9.1.0`.

  <Tabs id="native-file-changes-react-native-vector-icons" items={['Android', 'iOS']} />

  <Tab id="native-file-changes-react-native-vector-icons-0">
  The following step allows fonts being copied from this module at build time so that the fonts and JS are always in sync, making upgrades painless.
  ...
  <Tab id="native-file-changes-react-native-vector-icons-1">
  ...
  Edit the `Info.plist` and add a property called `Fonts` provided by application (or `UIAppFonts` if Xcode won't autocomplete/not using Xcode) and type in as shown below. It will look something like this -

  ![info-plist-vector-icons](/guides/rn-info-plist-vector-icons.png)
  ...
  If you are using `use_frameworks!` in your `Podfile` you need to dynamically load the icon font by doing `Icon.loadFont()` when boostrapping your application.

  <br/>
  <i>Note: You must be consuming React itself via CocoaPods for this to work, see [React Native documentation](https://reactnative.dev/docs/integration-with-existing-apps) on how to set that up.</i>
  </Tab>
...
```bash
npm install @100mslive/100ms-react-native
