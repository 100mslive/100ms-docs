...
`react-native-room-kit` package uses 'Inter' font family for texts. You need to add 'Inter' fonts in your app. You can download Inter font family from [Google Fonts](https://fonts.google.com/share?query=Inter).
...
1. add `react-native-gesture-handler` import statement __at the top__ in `index.js` file

```js{1} section=importGestureHandler
...
2. let's replace the code in the `App.js` file with the complete code sample below -
...
const App = () => {
  ...
  const handleRoomLeave = useCallback((reason) => {
    console.log(':: Reason for Leaving the Room > ', reason);
