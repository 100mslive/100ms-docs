...
After the Test run of your app is successful, You can install 100ms React Native library in your app.
...
- Allow camera, recording audio and internet permissions by adding the below snippet to the `AndroidManifest.xml` file (at the application tag level).
...
- You will also need to request camera, record audio permissions at runtime before you join a call or display a preview. Please follow the [Android Documentation](https://developer.android.com/training/permissions/requesting#request-permission) for runtime permissions.
...
- Allow camera, recording audio and internet permissions by adding the below snippet to the `info.plist` file.
...
```js
Join button Code Snippet here
```

### Step 3: Implement Room screen
...
```js
Room screen Initialization Code Snippet
```

### Step 4: Listen to Join and Peer Updates
...
```js
on_JOIN and ON_PEER_UPDATE listeners Code Snippet
```
...
```js
ON_TRACK_UPDATE listeners Code Snippet
```

### Step 6: Listen to Other Updates
...
```js
ON_ERROR listener Code Snippet
```
...
```js
Flatlist with list of PeerTrackNodes Code Snippet
```
