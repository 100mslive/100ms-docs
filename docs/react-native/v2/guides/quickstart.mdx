...
export const checkPermissions = async (permissions) => {
  try {
    ...
    for (let permission in permissions) {
      const permissionResult = results[permissions[permission]];

      const isBLConnectUnavailable = permissions[permission] === PERMISSIONS.ANDROID.BLUETOOTH_CONNECT && permissionResult === RESULTS.UNAVAILABLE;

      if (!(permissionResult === RESULTS.GRANTED || isBLConnectUnavailable)) {
