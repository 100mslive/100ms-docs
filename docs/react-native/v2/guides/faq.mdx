...
You can also refer to usage of `ON_TRACK_UPDATE` in our sample app for more clear example - 
...
## In ON_TRACK_UPDATE event TRACK_REMOVED event type comes after TRACK_ADDED event type, When multiple Role change request is done for multiple Roles

Scinerio explanation: When we change RoleA to RoleB
