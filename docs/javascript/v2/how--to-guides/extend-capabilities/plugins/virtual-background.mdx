...
> Note that `hmsActions.attachVideo` should be called on toggling virtual background. Refer [Render Video](/javascript/v2/how--to-guides/set-up-video-conferencing/render-video/overview#when-to-attachdetach-videos) section to see how to handle it.
...
- Use background images that match the video publish aspect ratio set from dashboard template.
  Eg: For 4:3 apsect ratio, image should be a minimum of 640*480 or higher.
- Image may look stretched or shrinked depending on video element size, so it should be closer to the video element size
  following the above aspect ratio
...
- For vidoes, all video formats that are supported by native `video` tag are supported.  We recommend to use mp4.
