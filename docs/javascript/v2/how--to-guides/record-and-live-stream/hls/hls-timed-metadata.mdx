...
if (Hls.isSupported()) {
  ...
  hls.on(Hls.Events.MANIFEST_LOADED, this.manifestLoadedHandler);
  hls.on(Hls.Events.LEVEL_UPDATED, this.levelUpdatedHandler);
  hls.on(Hls.Events.FRAG_CHANGED, this.fragChangeHandler);
