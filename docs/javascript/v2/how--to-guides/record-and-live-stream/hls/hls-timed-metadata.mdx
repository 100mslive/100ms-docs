...
```json
{
    "start_date": "UTC date", // The time when the event is received. Duplicate of START-DATE
...
>Please Note : Above implementation will be changed after we introducted HLS controller library.

## Limitions

- It’s hard to sync metadata with stream exactly due to the latency from ffmpeg creating the stream. To address this, we’ve added 2s the actual `START-DATE` attribute.
