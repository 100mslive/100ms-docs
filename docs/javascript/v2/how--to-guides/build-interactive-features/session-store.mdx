...
It is recommended to plan an interface for session store so that the app gets to know before hand what keys it needs to observe. If you're using Typescript, using an interface for session store also provides better type-safety and auto-completion. Read on to know more.
...
import { StyledMenuTile } from '@100mslive/react-ui';
