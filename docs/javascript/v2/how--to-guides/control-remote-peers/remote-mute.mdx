...
Can't let just anyone mute others. First you need to create a [role](/javascript/v2/foundation/templates-and-roles) with the permissions to end a room.

![Permissions - Remote mute/unmute](/docs/guides/permissions.png)

Use the [selectPermissions](/javascript/v2/how--to-guides/control-remote-peers/change-role#know-thy-permissions) selector to check whether the local peer has the
...
> 🚧 If the local peer doesn't have the required `mute`/`unmute` permission, the actions will throw an [HMSException error](/javascript/v2/how--to-guides/debugging/error-handling).

## Handling a mute/unmute request

![Remote Unmute Request](/docs/guides/remote-unmute.png)

Once the peer with adequate permissions calls `setRemoteTrackEnabled`, the local peer will receive a [notification](/javascript/v2/how--to-guides/listen-to-room-events/notifications).
...
-   For unmute request, use the information in `notification.data` to show a dialog to the user to ask if they want to accept
    the change and then apply the settings locally. The same as in a regular user [Mute/Unmute](/javascript/v2/how--to-guides/set-up-video-conferencing/mute).
    ...
    switch (notification.type) {
        ...
        case HMSNotificationTypes.CHANGE_MULTI_TRACK_STATE_REQUEST:
            ...
            if (enabled) {
                // Ask for consent using dialog or any other appropriate UI
                tracks.forEach(
                    async (track) => await hmsActions.setEnabledTrack(track.id, enabled)
                );
            } else {
                // Mute Request
                // Show toast to user
