...
- The room code of the room you want to join. You can get the room code from the [100ms dashboard](https://www.dashboard.100ms.live). This prop can be skipped if the roomId and role are being provided instead.

`logo` (optional)
- An image URL as a string which is displayed in the preview screen and header.

`authToken` (optional)
- Can be copied from the developer section of the [dashboard](https://www.dashboard.100ms.live). Read more about it [here](/docs/get-started/v2/get-started/security-and-tokens#auth-token-for-client-sdks).

`roomId` (optional unless roomCode is not being used)
- The room ID of the room you want to join. You can get the room ID from the [dashboard](https://www.dashboard.100ms.live). It should be specified with the role prop if the room code is not being provided.
