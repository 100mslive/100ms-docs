...
`roomCode` (optional)
- The room code of the room you want to join. You can get the room code from the [100ms dashboard](https://www.dashboard.100ms.live)

`logo` (optional)
- An image URL as a string which is displayed in the preview screen and header.

`authToken` (optional)
- Can be copied from the developer section of the [dashboard](https://www.dashboard.100ms.live)

`roomId` (optional)
- The room ID of the room you want to join. You can get the room ID from the [dashboard](https://www.dashboard.100ms.live). Works in combination with the role prop.

`role` (optional)
- A string specifying the role of the peer.
