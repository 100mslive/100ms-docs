...
TL;DR - Try out this complete example in [CodeSandbox](https://codesandbox.io/s/github/100mslive/100ms-examples/tree/main/web/js-quickstart).

<Codesandbox id="github/100mslive/100ms-examples/tree/main/web/js-quickstart" />

You can join the above room using room code from [dashboard](https://dashboard.100ms.live/dashboard). Refer [this](/concepts/v2/concepts/rooms/room-codes/room-codes) for more details about room code.
...
<StepsContainer id="sample-app-locally">
...
<html>
  ...
  <body>
    ...
    <form id="join">
      ...
      <div class="input-container">
        <input id="room-code" type="text" name="roomCode" placeholder="Room code" />
      </div>
...
2. Use the `Video Conferencing` template to create a room and test this app quickly.
3. Once the room is created, you will be shown a `Join Room` pop-up with options to join a room. Copy the room code shown in the pop-up.
4. Enter a user name and paste the room code copied in above step.
...
<video loop="true" autoplay="autoplay" muted>
    <source src="/docs/guides/quickstart-js-first-peer-join.mp4" type="video/mp4" />
</video>
<br />
...
<video loop="true" autoplay="autoplay" muted>
    <source src="/docs/guides/quickstart-js-second-peer-join.mp4" type="video/mp4" />
</video>
<br />
...
-   An instance of [HMSStore](/api-reference/javascript/v2/interfaces/HMSStore) that holds the complete state of the application such as details of all the participants. We can also visualize this state at any time using the [DevTools](https://chrome.google.com/webstore/detail/redux-devtools/lmhkpmbekcpmknklioeibfkpmmfibljd?hl=en) extension.
    -   This HMSStore is a single source of truth for the entire SDK. To get access to the data from this store, you have to call [hmsStore.subscribe()](/api-reference/javascript/v2/interfaces/IHMSStore#subscribe). It takes two parameters. A callback and a selector which tells the store which slice of state you want to subscribe to.
...
There is also a version of the above quickstart using our SDK from CDN directly. [here](https://codepen.io/triptu/pen/dymxmoR?editors=0010).
...
You can also check out our [React demo app](https://github.com/100mslive/100ms-web) built with more features like screenshare, virtual background, noise suppression (deprecated), audio/video playlist, etc.
