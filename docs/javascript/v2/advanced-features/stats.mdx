...
After setting up a video conferencing application, you'd want metrics to measure the call quality - what caused choppy audio, lost video frames, or who had a bad network. `getStats` has got your back. With `getStats` you could get statistics for the whole session and individual tracks of all the peers in the room.
...
This initializes the SDK to collect stats from WebRTC every 1 second and stores it in a Redux store called `HMSStatsStore`. This is similar to `HMSStore` in terms of retrieving data from the store.
...
Retrieving data is similar to how you do it with HMSStore. You have getState and subscribe methods and a bunch of useful selectors under selectHMSStats.
