...
After setting up a video conferencing application, you'd want metrics to measure the call quality - what caused choppy audio, lost video frames, or who had a bad network. <APILink type="class" name="HMSReactiveStore#get-stats">`getStats`</APILink> has got your back.
...
This initializes the SDK to collect stats from WebRTC every 1 second and stores it in a Redux store called <APILink>`HMSStatsStore`</APILink>. This is similar to <APILink>`HMSStore`</APILink> in terms of retrieving data from the store.
