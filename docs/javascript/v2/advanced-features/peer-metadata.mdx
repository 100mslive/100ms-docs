...
Note that `peer.metadata` is a string, it can be used to keep a jsonified string. The string will be converted properly
to a json object however if you use the selector described below.
This can be achieved by calling `hmsActions.changeMetadata(metadata)`, the below example shows implementing a toggle raise
...
async function toggleRaiseHand() {
    ...
    const newMetadata = { ...metadata, isHandRaise: !metadata.isHandRaised };
...
hmsNotifications.onNotification((notification) => {
    switch (notification.type) {
        case HMSNotificationTypes.METADATA_UPDATED:
            const peer = notification.data.isLocal;
