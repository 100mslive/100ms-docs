...
-   Objects as session metadata but the object should be JSON serializable.
...
-   Also after joining, session metadata is not available and needs to be fetched explicitly by the app using`populateSessionMetadata`(web) action.
-   No permission support - anyone can read/write session metadata.
