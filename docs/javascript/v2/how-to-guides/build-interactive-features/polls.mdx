...
The response contains an array of entries of type `HMSQuizLeaderboardEntry`, a boolean `hasNext` to know there are more entries and 
a summary object of type `HMSQuizLeaderboardSummary` containing details like the average score, average time taken to answer a question and more.
