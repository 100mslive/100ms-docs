...
interface HMSPlaylistItem<T> {
    ...
    /**
     * the URL to play from, can also be a URL created from local file for example: URL.createObjectURL(file)
     */
    url: string;
    /**
     * any additional info, for example composer, musician etc.
     */
    metadata?: T;
    /**
     * duration in seconds
