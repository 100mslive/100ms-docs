...
-   `preferCurrentTab`: Show the current tab first in the browser prompt but don't throw an error if any other tab is selected. Default value is `false`
...
-   `displaySurface`: Preselect the relevant tab in screenshare menu. Use `browser` for preferring a browser tab, `window` for application window, and `monitor` for full screen.
