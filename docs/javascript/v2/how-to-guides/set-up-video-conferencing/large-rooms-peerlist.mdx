...
> NOTE: The data given by the API might become stale, so it is recommended to poll the `iterator.findPeers()` for js and `loadPeers()` for React, so that if any peer
