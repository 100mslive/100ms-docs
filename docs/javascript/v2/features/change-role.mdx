...
3. Check what actions this role can perform. i.e can it change someone else's current role, end meeting, remove someone from the room. This can be discovered by using the `selectPermissions` selector.

In certain scenarios you may want to change your or someone else's role. Imagine an audio room with 2 roles "speaker" and "listener". Only someone with a "speaker" role can publish audio to the room while "listener" can only subscribe.
...
-   `peerId`: The peer ID whose role you want to change, this can be for either of local or remote peer.
-   `toRoleName`: The target role name.
-   `force`: Whether you want to change the role without asking through a request or give the peer a chance to accept/reject.
