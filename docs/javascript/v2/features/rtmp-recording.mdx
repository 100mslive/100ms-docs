...
3. **record**: _bool_. Optional, If recording is required this can be set as true. This value has no effect on streaming.
...
if (recordingState.browser.error) {
    console.error('error in starting recording - ', recordingState.browser.error);
}

console.log('is streaming going on - ', rtmpState.running);
console.log('time when rtmp was started - ', rtmpState.startedAt);
if (rtmpState.error) {
    console.error('error in starting RTMP - ', rtmpState.error);
}
```
