...
Additionally you can also start recording by passing in the recording field with two optional fields - 
1. `singleFilePerLayer`: if the final output should be one file or one file per hls layer, false by default
2. `hlsVod`: if video on demand needs to be turned on, false by default
...
async startHLS() {
    ...
    params.recording = {}; // if you want to enable recording as well
    params.recording = {singleFilePerLayer: false, hlsVod: false}; // to configure recording options
    try {
...
You may want to try out the end to end flow on our dashboard webapp before moving ahead with building in your app. To do
