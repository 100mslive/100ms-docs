...
These are returned as `type='error'` through the [notifications interface](Notifications). The type of `notification.data` is `HMSException`.
...
Most browsers have a restriction on audio autoplay where audio autoplay is allowed only if the user has interacted with the page.
If an autoplay error is received, UI can show a popup or a notification with button. Once a user click has happened confirming an interaction point,
