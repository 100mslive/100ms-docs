...
if you're looking for implementing something like raise hand do check out our documentation for [peer metadata](../advanced-features/peer-metadata).
...
Here's an implementation of the `useCustomEvent` hook.
...
const Confetti = () => {
    const onConfettiMsg = useCallback((config) => {
        // UI changes
    }, []);
    ...
    return <button onClick={sendConfetti}>Fire Confetti</button>;
};
