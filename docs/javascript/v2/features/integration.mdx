...
In case you want to use the CDN link, you can change the imports to come from cdn as present in the example [here](https://codepen.io/triptu/pen/dymxmoR?editors=0010). It's recommended to use a fixed version with CDN to avoid unintentional retrieval of a newer version with breaking changes. Although CDN is great for quickstart and prototyping, we highly recommend setting up a bundler for production to avoid version mismatches and a critical dependency on cdn provider.
