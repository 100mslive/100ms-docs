...
-   `image sharpening` - This is an action where the user video is sharpened, thus improving the overall quality. This will use a little additional resources (cpu/memory/battery).
...
2) enableSharpening, it is of boolean type (default: false)

```jsx
// background : {'blur' | image | 'none'}
const virtualBackground = new HMSVirtualBackgroundPlugin(background : {'blur' | image | 'none'}, enableSharpening : {true | false});
```
