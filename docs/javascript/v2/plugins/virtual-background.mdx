...
`pluginFrameRate`- Int - It can be used to set the output frame rate of virtual background plugin<br/>
 It is the rate at which the plugin is expected to process the video frames. If not set, virtual background will run on
 input frame rate. The user can specify this based on device type and CPU usage.<br/>
 **Range: 0 <= pluginFrameRate <= inputFrameRate**


## Start and Stop Virtual Background

Check [this section](#set-plugin-frame-rate-optional) for `pluginFrameRate` usage
