...
Check [this section](#specifying-custom-frame-rate-optional) to use custom pluginFrameRate
...
## Specifying custom frame rate(Optional)

`pluginFrameRate`- number - It can be used to set the processing frame rate of virtual background plugin.<br/>
 It is the rate at which the plugin is expected to process the video frames. If not set, processing will happen on every frame.
 The user can specify this based on device type and CPU usage. <br/>
 if you're seeing CPU spikes and continuous heat up on turning on virtual background, you may reduce the pluginFrameRate further.<br/>
 `For best performance` - pluginFrameRate = 15<br/>
 `For low end devices`  - pluginFrameRate = 7-10<br/>
