...
-  `image` - HTMLImageElement - This will replace the background to the image provided
-  'video' - HTMLVideoElement - This will replace the background to the video provided
-  `none` - String - This will remove the background effect from the video
-  `gifURL` - String - This is the URL of the gif with which the background will be replaced

```jsx
// background : {'blur' | image | video | gifURL | 'none'}
const virtualBackground = new HMSVirtualBackgroundPlugin(background : {'blur' | image | video | gifURL | 'none'});
```
...
It accepts string or HTMLImageElement or HTMLVideoElement as a parameter
Image/Video/Gif passed will get fit to the camera video by maintaining the aspect ratio.
If the aspect ratio of the background passed is not the same as the video, the image will be cropped to fit in the background.

```jsx
// background parameter is explained in Instantiate Virtual Background section
try{
  virtualBackground.setBackground(background : {'blur' | image | video | gifURL | 'none'});
}catch(err){
