...
-   `image sharpening` - This is an action where the user video is sharpened, thus improving the overall quality. This will use additional resources (cpu/memory/battery).
...
This accepts 2 parameters as input

1. background, it can be of 3 types:
...
2. enableSharpening (optional), it is of boolean type (default: false)

```js
const background: 'blur' | 'none' | HTMLImageElement = 'blur';
const enableSharpening: boolean = false;
// second parameter(enableSharpening) is optional
const virtualBackground = new HMSVirtualBackgroundPlugin(background, enableSharpening);
```
## Interfaces
