...
> the <a onClick={()=>{window.analytics.track("codesandbox.viewed",{path:window.location.pathname});window.open("https://codesandbox.io/s/github/triptu/100ms-face-filters","_blank")}}>codesandbox</a>.
