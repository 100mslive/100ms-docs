...
When using *room-composite browser recording* (formerly **beam recording**) with `meetingURL` using the specified or a newer SDK version:
...
- Valid when starting (room-composite recording, RTMP stream)[/javascript/v2/how-to-guides/record-and-live-stream/rtmp-recording] or (HLS stream)[/javascript/v2/how-to-guides/record-and-live-stream/hls/hls].
- No need to explain the presence of an extra peer (`beam`).
We highly recommend you to update any business logic dependent on beam's *peer webhook events* and instead use (*beam webhook events*)[/server-side/v2/how-to-guides/configure-webhooks/webhook#rtmp-streaming-browser-recording-events].
