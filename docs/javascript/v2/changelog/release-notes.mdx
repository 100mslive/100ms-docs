...
#### Fixed:
- `Uncaught exceptions` are handled properly for most cases.

#### Breaking:
- Video attach/detach and auto layer(adaptive bitrate) selection based on dimensions is moved to `@100mslive/hms-video-store`. Earlier this was only available with `useVideo` hook of `@100mslive/react-sdk`.
