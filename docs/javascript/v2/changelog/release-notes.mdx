...
- alpha release of `@100mslive/hls-player` is done.
 `@100mslive/hls-player` is currently a wrapper on hls.js with easy to use interface and few add-ons for [100ms's interactive live streaming feature](/javascript/v2/how--to-guides/record-and-live-stream/hls/hls).
...
#### Breaking:
- [`getAuthTokenByRoomCode`](/docs/api-reference/javascript/v2/interfaces/HMSActions#getauthtokenbyroomcode) in hmsActions now returns the token as a string in response.
...
#### Added:
- [`getAuthTokenByRoomCode`](/docs/api-reference/javascript/v2/interfaces/HMSActions#getauthtokenbyroomcode) in hmsActions to get token for room short code.
