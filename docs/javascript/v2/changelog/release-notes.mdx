...
Starting from this release, if the meeting url passed to start the recording, rtmp, or hls has sdk version same or above this release,
the beam peer will be completely hidden in the peer list. You won't see beam peer joins or leaves
on any platform (web, android, ios). No `peer.join.success` or `peer.leave.success` webhooks will be sent.
