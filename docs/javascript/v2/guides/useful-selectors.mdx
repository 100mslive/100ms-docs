...
Here's a list of useful Selector hooks that you might want to use while building your application. Before using these make sure you have initialized the SDK, you can refer to this [guide](/docs/javascript/v2/guides/javascript-quickstart#initializing-the-sdk).
...
`selectPeers` returns you an array of peers(remote peers and your local peer) present in the room.
...
What if you fail to join the room? How do you know if the user is reconnecting? This is where `selectRoomState` comes in handy, it returns you the current state of your room. You can refer to the [ENUMS](/docs/api-reference/javascript/v2/enums/HMSRoomState) types for reference.
...
You can get video stream of a user using `selectCameraStreamByPeer` it returns [HMSTrack](/docs/api-reference/javascript/v2/interfaces/HMSTrack) which you can use to call [attach/detach](/docs/javascript/v2/features/render-video) actions.
