...
```js
const audioOn = hmsStore.getState(selectIsPeerAudioEnabled);
const videoOn = hmsStore.getState(selectIsPeerVideoEnabled);
```
...
```jsx
const audioOn = useHMSStore(selectIsPeerAudioEnabled);
const videoOn = useHMSStore(selectIsPeerVideoEnabled);
```
