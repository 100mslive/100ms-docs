...
```bash section=Dependencies sectionIndex=1
...
```jsx:script.js section=SDK sectionIndex=1
...
```html:index.html section=Join sectionIndex=1 tab=HTML
<form id="join">
    <h2>Join Room</h2>
...
```jsx:script.js section=Join sectionIndex=1 tab=JS
...
```jsx:script.js section=Leave sectionIndex=1
function leaveRoom() {
    hmsActions.leave();
}
...
```jsx section=Fetch sectionIndex=1
...
```jsx:script.js section=Subscribe sectionIndex=1 tab=JS
import {
    HMSReactiveStore,
    selectPeers
...
```html:index.html section=Subscribe sectionIndex=1 tab=HTML
...
```jsx{3-16,20-20,23-47}:script.js section=Leave sectionIndex=2
const peersContainer = document.getElementById('peers-container');
...
```jsx:script.js section=Change sectionIndex=1
import {
...
```jsx:script.js section=Mute sectionIndex=1 tab=JS
import {
...
```html section=Mute sectionIndex=1 tab=HTML
...
```jsx{8}:script.js section=Leave sectionIndex=2
muteVid.addEventListener("click", () => {
  const videoEnabled = hmsStore.getState(selectIsLocalVideoEnabled);
...
```jsx{10,28}:script.js section=Mirror sectionIndex=1 tab=JS
function renderPeers(peers) {
  peersContainer.innerHTML = "";
...
```css:styles.css section=Mirror sectionIndex=1 tab=CSS
.local.peer-video {
    transform: scaleX(-1);
}
```
