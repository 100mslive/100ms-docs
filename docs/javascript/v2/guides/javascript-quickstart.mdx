...
TL;DR - You can find the link to the complete example over [here](https://codesandbox.io/s/hms-js-quickstart-5rmes). There is also a version of the quickstart using CDN [here](https://codepen.io/triptu/pen/dymxmoR?editors=0010).
...
```jsx:script.js section=InitializingTheSDK sectionIndex=1
...
> Note: join is async from this [version](../release-notes/release-notes#2022-09-13)
...
```jsx:script.js section=JoiningRoom sectionIndex=1 tab=JS
...
```jsx:script.js section=LeavingTheRoom sectionIndex=1
function leaveRoom() {
    hmsActions.leave();
}
...
```jsx section=FetchingInformationFromState sectionIndex=1
...
```jsx:script.js section=SubscribingToTheState sectionIndex=1 tab=JS
import {
    HMSReactiveStore,
    selectPeers
...
```jsx{3-16,20-20,23-47}:script.js section=SubscribingToTheState sectionIndex=2
const peersContainer = document.getElementById('peers-container');
...
```jsx:script.js section=ChangingUIBasedOnConnectionState sectionIndex=1
import {
...
```jsx:script.js section=MutingUnmutingLocalTracks sectionIndex=1 tab=JS
import {
...
```html section=MutingUnmutingLocalTracks sectionIndex=1 tab=HTML
...
```jsx{8}:script.js section=MutingUnmutingLocalTracks sectionIndex=2
muteVid.addEventListener("click", () => {
  const videoEnabled = hmsStore.getState(selectIsLocalVideoEnabled);
...
```jsx{10,28}:script.js section=MirroringLocalVideo sectionIndex=1 tab=JS
function renderPeers(peers) {
  peersContainer.innerHTML = "";
...
```css:styles.css section=MirroringLocalVideo sectionIndex=1 tab=CSS
.local.peer-video {
    transform: scaleX(-1);
}
```
