...
You might notice that `hmsStore.subscribe()` is used in many places. Let's take a moment to discuss this function.
hmsStore is a single source of truth for all SDK related data. To get access to this store, we have to `subscribe()` to
the store. It takes two arguments, a callback and a selector to tell what data in the store you want to subscribe.
