---
title: Clubhouse clone in React
nav: 2.4
---

## Getting Started

In this guide we will be building Clubhouse clone in React and using 100ms React SDKs by which you can have audio chats with your friends.
...
I have created a starter project based on [CRA + Tailwind](https://tailwindcss.com/docs/guides/create-react-app). To make things easier and to help us focus on adding the core functionality i already created all UI React Components we will be using in the project.

```bash
git clone <GIT_REPO_URL> clubhouse-clone
```
...
We will be needing `token_endpoint` & `room_id` from 100ms Dashboar to get these credentials you 1st need to create an account at [100ms Dashboard](https://dashboard.100ms.live/) after your account is setup head over to the [Developer Section](https://dashboard.100ms.live/developer). You can find your `token_endpoint` there.

![Developer Section](/guides/token/credential.png)

### Creating Roles

Before we create a room we will create a custom app , you can find it [here](https://dashboard.100ms.live/dashboard).
Click on "Add a new App", you will be asked to choose a template choose "Create your Own".

![Create Custom App](/guides/clubhouse/create-custom.png)
...
-   `moderator`: able to listen and speaker , and able change other's role, muting.

We will create listener's role 1st , we uncheck all publish strategies and then click on save.

![Listener role](/guides/clubhouse/listener-role.png)

Now we will create `speaker` role click on "Add a new role" we will uncheck `Can share video` & `Can share screen` we will keep `Can share audio` checked.

For the `moderator` role we will do the same as `speaker` and also modify it's permissions by checking `Can change any participant's role` & `Can mute any participant` in the permissions sections.
...
![Custom App Created](/guides/clubhouse/custom-app.png)

## Creating Room

In the last step we created our custom app and setup custom roles , in this step we will create a `room_id`.

To create a room head over to [Create Room](https://dashboard.100ms.live/create-room) page in 100ms Dashboard click on "Create Room" make sure to select the Custom App that you created in the last step. Fill out Room Name and click "Create Room". After creating you will be routed to Room Details were you can find your `room_id`

![Create Room](/guides/clubhouse/create-room.png)

Awesome now that we have `token_endpoint` and `room_id` we will add it in our app.
...
![Join Screen](/guides/clubhouse/join.png)
...
![Joining a Room](/guides/clubhouse/join-room.gif)
...
![Rendering Peer's Tiles](/guides/clubhouse/tile.gif)
...
![Leave Room](/guides/clubhouse/leave.gif)
...
You should have observed this in Zoom , Meet , Clubhouse Spaces that when someone is speaking his/her Tile gets highlighted based on the intensity of the speaker's audio. You can retrieve the audio level of the peer using `selectPeerAudioByID` selector.
...
![Audio Levels](/guides/clubhouse/level.gif)
...
![Change Roles](/guides/clubhouse/role.gif)
...
![Add Chat](/guides/clubhouse/chat.gif)
