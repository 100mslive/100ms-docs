...
- Roomkit Web: Added a new package that exports `HMSPrebuilt` from roomkit-react as a web component. [Read more](https://www.npmjs.com/package/@100mslive/roomkit-web)

### Fixed:
- Network indicator tooltip in the participant list would get hidden for the last peer.
- Prebuilt: Preview would show go live CTA even when browser recording was running, resulting in failure to go live.
...
- `hasNext` is now part of in [`usePaginatedParticipants`](/javascript/v2/react-hooks/interfaces/usePaginatedParticipantsResult)

### Fixed:
- Roomkit Prebuilt: Internal React Router is now removed. This will make it easy to use in next.js and remix
- Roomkit Prebuilt: PDF share
