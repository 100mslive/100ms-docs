...
- Name input in the preview is now disabled if the `userName` prop is provided to the Roomkit Prebuilt component. The `Change Name` option is also disabled from the local peer tile menu
