...
- In Roomkit Prebuilt, added a new field - `track_type` - in `HMSException` when there is a Capture Device error
...
- If Skip Preview was enabled, then the previous session's device selection was not preserved
- If the peer stays on Preview screen for more than 15 minutes, then Rejoin page is now shown instead of loading screen.
