...
#### Added:
- Added `HMSEffectsPlugin` class to integrate effects SDK for virtual background in custom implementation
- Roomkit Prebuilt: Quiz leaderboard also shows the summary - average score, participation percentage, total correct responses
- Added selectors for getting the effects SDK key for virtual background and checking if it is enabled: `selectIsEffectsEnabled` and `selectEffectsKey`
- Added batching for chat messages to improve performance under heavy loads (10+ messages/second)
