...
Released: `@100mslive/hms-video-store@0.15.1`, `@100mslive/react-sdk@0.13.1`, `@100mslive/hls-player@0.6.1`, `@100mslive/roomkit-react@0.6.1`, `@100mslive/hms-whiteboard@0.3.1`, `@100mslive/hms-virtual-background@1.16.1`, `@100mslive/react-icons@0.13.1`

### Fixed:

-   A remote peer could stay silent for a whole session, or a recording could have no video, when the SFU dropped the first subscribe request of the session — it is now retried
-   Switching mics, or recovering from a mic interruption, could leave the mic silent with an audio plugin such as Krisp failing with `AddAlreadyInProgress`
...
-   A remote peer could stay silent for the whole session, or their video could never start — the first subscribe request after joining is sent the instant the API data channel opens, and a request sent in that window never reaches the server. It then went unanswered with no timeout, so the caller waited for the rest of the session: the audio element was never attached, and the server was never told to start sending video. Subscribe and video quality requests now time out after 10 seconds and are retried
-   Audio playout no longer waits on the subscribe-state reply before attaching the audio element — audio is subscribed by default, so a lost or slow reply can no longer leave a peer permanently unheard
