...
-   `HMSRoomProvider` now calls `leave()` on unmount when it owns the store, so apps that mount the provider conditionally no longer leak a live room when the component tree is torn down
-   `VideoElementManager` observer callbacks are now guarded after track destruction, preventing `Cannot read properties of null` errors when a track is removed while its intersection or resize observers are still firing
-   Local video track lifecycle hardening: `processedTrack` is now stopped only after `sender.replaceTrack` lands, and several small track-surface fixes prevent the published track from being closed before the SFU has switched over
-   Signaling robustness: `pingPongLoop` now uses a generation guard on reschedule so stale ping loops can no longer race a fresh socket, and `retrySignalDisconnectTask` bails out early when `joinParameters` has been cleared (avoids reconnect attempts after a deliberate leave)
-   Transport fixes for ICE/SDP edge cases: corrected the publisher/subscriber role check in `handleIceConnectionFailure`, and the `onOffer` catch path no longer writes connection state directly — failures now flow through the regular reconnection state machine
