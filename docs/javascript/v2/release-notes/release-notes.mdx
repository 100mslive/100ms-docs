...
### Fixed:
- Prebuilt: Audio only config is removed to prevent accidental selection which results in black tiles in the call

### Added:
- Prebuilt now supports more chat based controls like pausing, hiding messages, blocking peers, and private/public messaging which can be configured from the dashboard
- Name input in the preview is now disabled if the `userName` prop is provided to the prebuilt component
...
### Fixed:
- Resolved "Leave called before join error" which occurred intermittently during preview
- Fixed audio stats showing up in the video section of subscriber stats
- Fixed poll votes being lost on page reload

### Added:
- Prebuilt: Upto 3 messages can now be pinned in the chat
