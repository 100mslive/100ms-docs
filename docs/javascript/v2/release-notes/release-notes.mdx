...
-   Incorrect device showing in device selection when initially joined as muted.

Breaking:

-   All the requests(RoleChange, track mute/unmute etc) will have peer/requestBy as optional now.

> NOTE: if you are planning to use api's to send chat messages, the sdk version should be minimun at this version.
...
-   Exposed Styleable Pagination component from react-ui
...
-   Errors occured during screenshare(permission not given etc.) will be sent via notification as well now in
    addition to being thrown on the method call
-   `hmsActions.refreshDevices()` to refresh the device list from UI side. This can be done for e.g. if the UI is
...
-   A exception will be thrown when preview/join is called if there is no WebRTC support now(for e.g. disable via an extension)
...
-   Support for Multiple [video plugins](../plugins/custom-video-plugin)
