...
Released: `@100mslive/hms-video-store@0.12.5`, `@100mslive/react-sdk@0.10.5`, `@100mslive/hls-player@0.3.5`, `@100mslive/roomkit-react@0.3.5`

### Added:

- Roomkit Prebuilt: Hand raise config set via the customiser is honored
- Roomkit Prebuilt: Tile UI for SIP peer
- Roomkit Prebuilt: Pause/freeze duration to the tile in Stats For Nerds

### Fixed :

- Newly connected device not being used
- Cumulative stats for packets sent and lost 
- Updated the protocol spec version
- Roomkit Prebuilt: HLS stream dimensions are not fixed
- Roomkit Prebuilt: Chat toggle in fullscreen landscape mode
- Roomkit Prebuilt: Whiteboard is now disabled during screenshare
- Roomkit Prebuilt: Mismatch in condition breaks PDF share
...
- Roomkit Prebuilt: Empty block in options when PIP is not supported
