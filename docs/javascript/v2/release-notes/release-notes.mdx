...
-   `onTrackInterruption` on the update and preview listeners, with `TRACK_INTERRUPTION_START` and `TRACK_INTERRUPTION_END` notifications for `useHMSNotifications` — use it to tell the user when the OS or another app has taken the mic or camera, and when it comes back
-   Roomkit Prebuilt: a dialog naming the device that was interrupted

### Fixed:

-   The mic and camera now recover after an OS interruption — an incoming call, a native VoIP app, or Safari being backgrounded on iOS previously left the device silently dead, with remote peers seeing a frozen tile or treating the peer as muted
