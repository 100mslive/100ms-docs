...
You can use the `Video aspect ratio` config in template configuration on [dashboard](https://dashboard.100ms.live/dashboard) or use the [update template server API](/server-side/v2/policy/update-a-template) with the respective height and width for the particular role using the [publishParams object](/server-side/v2/policy/create-template-via-api#publish-params-object).
