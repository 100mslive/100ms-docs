...
Yes, please check [webhooks](/docs/server-side/v2/foundation/webhook).

## How do I record a room?

We have two types of recordings available, [SFU](/docs/server-side/v2/features/recording) and [Browser](/docs/server-side/v2/features/rtmp-streaming-and-browser-recording).
You can also start the latter from the [SDK](/docs/javascript/v2/features/rtmp-recording).
...
when done will also show up in the redux devtools [extension](/docs/javascript/v2/debugging/debugging). Some things we have seen in the past -
...
the docs for [render video](/docs/javascript/v2/features/render-video) for more details.
...
to detect and resolve this given in more details [here](/docs/javascript/v2/features/error-handling#handling-autoplay-error).

## Is it possible to do RTMP out, live stream a room to YouTube, Twitch, Wowza?

Yes, you can achieve it both from [server-side APIs](/docs/server-side/v2/features/rtmp-streaming-and-browser-recording) or [SDK](/docs/javascript/v2/features/rtmp-recording).
...
More details [here](/docs/javascript/v2/features/sub-degradation).

## Can I store extra information with a peer?

Yes you can store [peer metadata](/docs/javascript/v2/advanced-features/peer-metadata) for a peer. The initial value can be provided at the time of join,
and can be modified post join.

## How do I implement Raise Hand?

You can do using [peer metadata](/docs/javascript/v2/advanced-features/peer-metadata).
...
should treat it similar to the track turning off for purpose of displaying avatar etc. More details [here](/docs/javascript/v2/features/sub-degradation).
...
Yes, the core sdk is framework agnostic, you can follow the [JS Quickstart](/docs/javascript/v2/guides/javascript-quickstart) to learn the basics. The quickstart
...
Please see [reaching out](/docs/javascript/v2/debugging/debugging#reaching-out).

## Can I create room using API?

[Yes](/docs/server-side/v2/features/room).

## Can I disable a room?

[Yes](/docs/server-side/v2/features/room#disable-room). You can also do this while ending the room using the [sdk](/docs/javascript/v2/features/end-room).

## Is it possible to create and manage roles using APIs?

[Yes](/docs/server-side/v2/foundation/templates-and-roles).

## Does the SDK remembers input/output device selection for future joins?

Yes, just make sure you pass the `rememberDeviceSelection` as true in the [join config](/docs/javascript/v2/features/join).

## Can I implement custom events to broadcast or sent to a specific person in the room?

Yes, you can do so using our [messaging system](/docs/javascript/v2/features/chat#custom-events).
...
[Yes](/docs/javascript/v2/advanced-features/volume-control).

## Can I process video before sending over to others in the room?

Yes, you can write [custom video plugins](/docs/javascript/v2/plugins/custom-video-plugins).

## How to disable console logs if I'm using the web SDK?

Please follow [setting log level](/docs/javascript/v2/features/log-level).
