...
4. Fetch room join ([joinedAt](https://www.100ms.live/docs/api-reference/javascript/v2/interfaces/HMSPeer#joinedat)) timestamp for each waiting room participant
...
### Create roles to set up waiting room

-   This starter kit configures your new app with [two roles](https://www.100ms.live/docs/javascript/v2/foundation/templates-and-roles):
    ...
    -   `guest`: This role represents the guest,
        ...
        -   Publishes tracks audio/video/screen tracks
...
-   You need to add a new role, `waiting-room` to allow the participants to join a waiting room
    ...
    -   And shouldn't have any permissions like the host
        ...
        <video loop="true" autoplay="autoplay" controls="controls" id="vid" muted>
            <source src="/guides/waiting-room/waiting-room.mp4" type="video/mp4" />
        </video>
        <br />
...
3. Go to the peer list in the host view and click on the three-dots menu > change role > waiting room to the guest.
    ...
    <br />
    <video loop="true" autoplay="autoplay" controls="controls" id="vid" muted>
        <source src="/guides/waiting-room/move-participant.mp4" type="video/mp4" />
    </video>

> **Note**: Force the role change action rather than requesting the participant. Change role action leverages the [change role](https://www.100ms.live/docs/javascript/v2/how--to-guides/control-remote-peers/change-role) client-side API.
...
-   [Create a template](https://www.100ms.live/docs/server-side/v2/policy/create-template-via-api) with host, guest and waiting room roles
    -   You can set the publish/subscribe strategies and permissions using the [publishParams](https://www.100ms.live/docs/server-side/v2/policy/create-template-via-api#publishparams-object), [subscribeParams](https://www.100ms.live/docs/server-side/v2/policy/create-template-via-api#subscribeparams-object) and [permissions](https://www.100ms.live/docs/server-side/v2/policy/create-template-via-api#permissions-object) objects.
    -   If you're trying to use an existing template, you can create a new role `waiting room` using the [create a role API](https://www.100ms.live/docs/server-side/v2/policy/create-update-role) to allow participants to join a waiting room.
-   Once the template is set up and a session has started with a host participant as a host and waiting room participants, fetch the room join ([joined_at](https://www.100ms.live/docs/server-side/v2/active-rooms/list-peers)) timestamp for each participant.
-   Use the [update a peer API](https://www.100ms.live/docs/server-side/v2/active-rooms/update-a-peer) with the value for `role` argument as `guest` to move the participant from waiting room to main room.
