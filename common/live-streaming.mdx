...
1. Select if viewers can be temporarily brought on stage to enable the broadcaster and co-broadcaster to temporarilty invite viewers on stage, and remove them later.
...
-   `viewer-delayed-stream`: This role represents an off stage particpant (HLS) with a delayed stream, who subscribes to the composite live stream and can interact using messaging
